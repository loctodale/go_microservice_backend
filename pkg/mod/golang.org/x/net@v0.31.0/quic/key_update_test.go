...
func TestKeyUpdatePeerUpdates(t *testing.T) {
	...
	// Peer initiates a certs update.
	tc.sendKeyNumber = 1
	tc.sendKeyPhaseBit = true
	tc.writeFrames(packetType1RTT, debugFramePing{})

	// We update to the new certs.
	tc.advanceToTimer()
	tc.wantFrameType("conn ACKs last packet",
		packetType1RTT, debugFrameAck{})
	tc.wantFrame("first packet after a certs update is always ack-eliciting",
		packetType1RTT, debugFramePing{})
	if got, want := tc.lastPacket.keyNumber, 1; got != want {
		t.Errorf("after certs rotation, conn sent packet with certs %v, want %v", got, want)
	}
	if !tc.lastPacket.keyPhaseBit {
		t.Errorf("after certs rotation, conn failed to change Key Phase bit")
	}
	...
	// Peer initiates a second certs update.
	tc.sendKeyNumber = 2
	...
	if tc.lastPacket.keyPhaseBit {
		t.Errorf("after second certs rotation, conn failed to change Key Phase bit")
	}
	tc.wantIdle("conn has nothing to send")
}
...
func TestKeyUpdateAcceptPreviousPhaseKeys(t *testing.T) {
	...
	// Peer initiates a certs update, skipping one packet number.
	...
	// We receive the previously-skipped packet in the earlier certs phase.
	tc.peerNextPacketNum[appDataSpace] = pnum0
	tc.sendKeyNumber = 0
	tc.sendKeyPhaseBit = false
	tc.writeFrames(packetType1RTT, debugFramePing{})

	// We ack the reordered packet immediately, still in the new certs phase.
...
func TestKeyUpdateLocallyInitiated(t *testing.T) {
	const updateAfter = 4 // initiate certs update after 1-RTT packet 4
	tc := newTestConn(t, serverSide)
	...
	for {
		...
		if got, want := tc.lastPacket.keyNumber, 0; got != want {
			t.Errorf("before certs update, conn sent packet with certs %v, want %v", got, want)
		}
		if tc.lastPacket.keyPhaseBit {
			t.Errorf("before certs update, keyPhaseBit is set, want unset")
		}
	}
	if got, want := tc.lastPacket.keyNumber, 1; got != want {
		t.Errorf("after certs update, conn sent packet with certs %v, want %v", got, want)
	}
	if !tc.lastPacket.keyPhaseBit {
		t.Errorf("after certs update, keyPhaseBit is unset, want set")
	}
	tc.wantFrame("first packet after a certs update is always ack-eliciting",
		packetType1RTT, debugFramePing{})
	...
	tc.wantFrame("first packet after a certs update is always ack-eliciting",
		packetType1RTT, debugFramePing{})
	if got, want := tc.lastPacket.keyNumber, 2; got != want {
		t.Errorf("after peer certs update, conn sent packet with certs %v, want %v", got, want)
	}
	if tc.lastPacket.keyPhaseBit {
		t.Errorf("after peer certs update, keyPhaseBit is unset, want set")
	}
}
