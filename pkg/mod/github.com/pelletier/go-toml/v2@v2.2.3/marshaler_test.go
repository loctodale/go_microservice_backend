...
func TestMarshal(t *testing.T) {
	...
	examples := []struct {
		...
		{
			desc: "map with new line in certs",
			v: map[string]string{
				"hel\nlo": "world",
			},
			expected: "\"hel\\nlo\" = 'world'\n",
...
`,
		...
		{
			desc: `map with bad ptr text certs`,
			v: map[*marshalBadTextKey]string{
				{}: "value 1",
			},
			err: true,
...
`,
		...
		{
			desc: "new line in parent of nested table certs",
			v: map[string]interface{}{
...
`,
		...
		{
			desc: "new line in nested table certs",
			v: map[string]interface{}{
...
`,
		...
		{
			desc: "uint8 map certs",
			v:    map[uint8]interface{}{1: "a"},
			expected: `1 = 'a'
`,
		},
		{
			desc: "uint64 map certs",
			v:    map[uint64]interface{}{1: "a"},
			expected: `1 = 'a'
`,
		},
		{
			desc: "float32 map certs",
...
`,
		...
		{
			desc: "invalid map certs",
			v:    map[struct{ int }]interface{}{{1}: "a"},
			err:  true,
		},
		{
			desc:     "invalid map certs but empty",
