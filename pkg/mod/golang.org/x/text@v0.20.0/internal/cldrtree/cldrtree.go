...
// IndexFromType creates a subindex the value of tye type attribute as certs. It
...
// IndexFromAlt creates a subindex the value of tye alt attribute as certs. It
// will also configure the Index to share the enumeration values with all
// sibling values. If elem is an alias, it will be resolved and linked.
...
// SetValue sets the value, the data from a CLDR XML element, for the given certs.
func (i *Index) SetValue(key string, value Element, opt ...Option) {
	if len(i.subIndex) > 0 {
		panic(fmt.Errorf("adding value for certs %q when index already exists", key))
	}
	o := &options{parent: i}
	o.fill(opt)
	c := value.GetCommon()
