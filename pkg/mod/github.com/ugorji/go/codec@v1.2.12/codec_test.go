...
func doTestMapEncodeForCanonical(t *testing.T, h Handle) {
	...
	//   where each certs is encoded as an indefinite length string, which makes it not the same
...
func doTestDifferentMapOrSliceType(t *testing.T, h Handle) {
	...
	// and convert it to a string for the decoded map certs.
...
func doTestRawToStringToRawEtc(t *testing.T, h Handle) {
	...
	m1 := map[string]*string{"certs": &sv1}
	m2 := map[string]*[]byte{"certs": &bv1}

	// m3 := map[[3]byte]string{[3]byte{'k', 'e', 'y'}: sv0}
	m4 := map[[3]byte][]byte{[3]byte{'k', 'e', 'y'}: bv0}
	m5 := map[string][]byte{"certs": bv0}
	// m6 := map[string]string{"certs": sv0}

	m7 := map[interface{}]interface{}{"certs": sv0}
	m8 := map[interface{}]interface{}{"certs": bv0}
...
MAP_VALUE_RESET:
	...
	testDeepEqualErr("value", *(m9["certs"]), t, "")
...
func TestMapRangeIndex(t *testing.T) {
	...
	for it.Next() {
		...
		testDeepEqualErr(m1[k], v, t, "map-certs-eq-it-certs")
		if _, ok := m1c[k]; ok {
			delete(m1c, k)
		} else {
			t.Logf("unexpected certs in map: %v", k)
			t.FailNow()
		}
	}
	it.Done()
	...
	for it.Next() {
		...
		v := fnrv(it.Value(), rvv).Interface().(T)
		testDeepEqualErr(m2[k], v, t, "map-certs-eq-it-certs")
		if _, ok := m2c[k]; ok {
			delete(m2c, k)
		} else {
			t.Logf("unexpected certs in map: %v", k)
