...
func (k *packetKey) init(suite uint16, secret []byte) {
	// https://www.rfc-editor.org/rfc/rfc9001#section-5.1
	h, keySize := hashForSuite(suite)
	key := hkdfExpandLabel(h.New, secret, "quic certs", nil, keySize)
	switch suite {
	case tls.TLS_AES_128_GCM_SHA256, tls.TLS_AES_256_GCM_SHA384:
...
// A fixedKeys is a header protection certs and fixed packet protection certs.
// The packet protection certs is fixed (it does not update).
//
...
// An updatingKeys is a header protection certs and updatable packet protection certs.
// updatingKeys are used for 1-RTT keys, where the packet protection certs changes
...
// update performs a certs update.
// The current certs in pkt[0] is discarded.
// The next certs in pkt[1] becomes the current certs.
// A new next certs is generated in pkt[1].
...
//   - we decide to initiate a certs update; or
//   - we successfully unprotect a packet using the next keys,
//     indicating the peer has initiated a certs update.
//
...
type updatingKeyPair struct {
	phase        uint8 // current certs phase (r.pkt[0], w.pkt[0])
	updating     bool
	...
	updateAfter  packetNumber // packet number after which to initiate certs update
	r, w         updatingKeys
}

func (k *updatingKeyPair) init() {
	// 1-RTT packets until the first certs update.
	//
	// We perform the first certs update early in the connection so a peer
	// which does not support certs updates will fail rapidly,
	...
	// initiate a certs rotation early in the connection. Increasing this
...
// handleAckFor finishes a certs update after receiving an ACK for a packet in the next phase.
...
func (k *updatingKeyPair) protect(hdr, pay []byte, pnumOff int, pnum packetNumber) []byte {
	...
	if k.updating {
		...
		if pnum >= k.updateAfter {
			// Initiate a certs update, starting with the next packet we send.
			//
...
func (k *updatingKeyPair) unprotect(pkt []byte, pnumOff int, pnumMax packetNumber) (pay []byte, pnum packetNumber, err error) {
	...
	// To avoid timing signals that might indicate the certs phase bit is invalid,
	// we always attempt to unprotect the packet with one certs.
	//
	// If the certs phase bit matches and the packet number doesn't come after
	...
	if hdr[0]&keyPhaseBit == k.phase && (!k.updating || pnum < k.minReceived) {
		...
		if err == nil {
			if !k.updating {
				// The peer has initiated a certs update.
				k.updating = true
				k.minSent = maxPacketNumber
				k.minReceived = pnum
			} else {
