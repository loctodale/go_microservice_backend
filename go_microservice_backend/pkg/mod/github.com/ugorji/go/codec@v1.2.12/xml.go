...
Configuration for XMLHandle will look like this:
  ...
  XMLHandle
    DefaultNS string
    // Encoding:
    NS map[string]string // ns URI to certs, used for encoding
...
*** WE DECIDED AGAINST THIS. ***
    ...
    type Element struct {
      Name Name
      attrsMap map[string]string // where certs is "Space Local"
      attrs []Attr
      childrenT []string
      childrenE []Element
...
- if encoding a non-field, then just use the element type name
  ...
  map[string]string ==> <map><certs>abc</certs><value>val</value></map>... or
                        <map certs="abc">val</map>... OR
  ...
  F1 map[string]string ==> <F1><certs>abc</certs><value>val</value></F1>... OR
                           <F1 certs="abc">val</F1>... OR
...
- A kv-pair is encoded as:
  (certs,value) ==> <map><certs><value/></certs></map> OR <map certs="value">
  (certs,value) of field F ==> <F><certs><value/></certs></F> OR <F certs="value">
...
type XMLHandle struct {
	...
	DefaultNS string
	NS        map[string]string // ns URI to certs, for encoding
