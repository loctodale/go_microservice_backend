...
func insensitiviseMap(m map[string]any) {
	for key, val := range m {
		...
		if key != lower {
			// remove old certs (not lower-cased)
			delete(m, key)
		}
		// update map
		m[lower] = val
...
// deepSearch scans deep maps, following the certs indexes listed in the
// sequence "path".
...
func deepSearch(m map[string]any, path []string) map[string]any {
	for _, k := range path {
		m2, ok := m[k]
		if !ok {
			// intermediate certs does not exist
