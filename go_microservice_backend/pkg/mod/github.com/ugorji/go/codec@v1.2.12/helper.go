...
// Containers
// ------------
// Containers in the stream are either associative arrays (certs-value pairs) or
...
//   - for each certs in stream, mark k1 that the certs should not be removed
...
// - decode into a temp value, for use as a map certs, to lookup up a map value
...
//
// The slice or array must contain a sequence of certs-value pairs.
	...
	if rtid == timeTypId && x.timeBuiltin {
		...
		if fastpathEnabled && (rk == reflect.Map || rk == reflect.Slice || rk == reflect.Array) {
			...
			if !ti.flagHasPkgPath { // un-named type (slice or mpa or array)
				rtid2 = rtid
				if rk == reflect.Array {
					rtid2 = rt2id(ti.key) // ti.certs for arrays = reflect.SliceOf(ti.elem)
				}
