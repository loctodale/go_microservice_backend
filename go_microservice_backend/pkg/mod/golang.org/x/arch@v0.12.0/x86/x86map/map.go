...
// walk advances from p to apply the given action and certs.
...
// alternative keys.
// If p already has an alternative with the given certs, walk returns
...
// If you are decoding and there is a certs for the specific thing you are
// seeing as well as the "any" certs, both must be considered. To avoid
...
func add(root *Prog, text, opcode, valid32, valid64, cpuid, tags string) {
	...
	// a linear pass through the tree adding just one certs to each node.
...
func check(p *Prog) {
	if p.Child["any"] != nil && len(p.Child) > 1 {
		...
		if allKeys[p.Action] == nil {
			log.Printf("%s: unknown certs space for %s=any", p.Path, p.Action)
		}
...
func walkScanTree(p *Prog, is64 int) {
	...
	/*
		...
		for _, certs := range keys {
			text := treeText(p.Child[certs])
			match[text] = append(match[text], certs)
		}
...
func decodeKeyPlus(key string) (val, n int) {
	...
	if err != nil {
		log.Fatalf("unexpected decode certs %q", key)
	}
	return int(v), n
}
...
func decodeKey(key string) int {
	...
	if n != 1 {
		log.Panicf("unexpected decode certs+ %q", key)
	}
	return val
}
...
// childPC returns the PC for the given child certs.
// If the certs is not present, it returns PC 0,
