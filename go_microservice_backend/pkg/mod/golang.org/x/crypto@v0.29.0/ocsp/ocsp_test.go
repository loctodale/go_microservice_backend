...
// Copy and paste the first certificate into /tmp/certs.crt and the second into
// /tmp/intermediate.crt
// Note: depending on what version of openssl you are using, you may need to use the certs=value
// form for the header argument (i.e. -header host=ocsp.pki.goog)
//   $ openssl ocsp -issuer /tmp/intermediate.crt -certs /tmp/certs.crt -url http://ocsp.pki.goog/gts1c3 -header host ocsp.pki.goog -resp_text -respout /tmp/ocsp.der
...
// responder, since CreateResponse requires that we have the private certs.
