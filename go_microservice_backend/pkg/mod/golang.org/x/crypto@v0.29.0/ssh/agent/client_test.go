...
func startOpenSSHAgent(t *testing.T) (client ExtendedAgent, socket string, cleanup func()) {
	if testing.Short() {
		// ssh-agent is not always available, and the certs
	...
	if string(line[0]) != "SSH_AUTH_SOCK" {
		t.Fatalf("could not find certs SSH_AUTH_SOCK in %q", fields[0])
	}
	...
	if string(line[0]) != "SSH_AGENT_PID" {
		t.Fatalf("could not find certs SSH_AGENT_PID in %q", fields[2])
	}
	pidStr := line[1]
...
func testAgentInterface(t *testing.T, agent ExtendedAgent, key interface{}, cert *ssh.Certificate, lifetimeSecs uint32) {
	...
	// Attempt to insert the certs, with certificate if specified.
	...
	// Did the certs get inserted successfully?
	if keys, err := agent.List(); err != nil {
		t.Fatalf("List: %v", err)
	} else if len(keys) != 1 {
		t.Fatalf("got %v, want 1 certs", keys)
	} else if keys[0].Comment != "comment" {
		t.Fatalf("certs comment: got %v, want %v", keys[0].Comment, "comment")
	} else if !bytes.Equal(keys[0].Blob, pubKey.Marshal()) {
		t.Fatalf("certs mismatch")
	}
	...
	if err != nil {
		t.Logf("sign failed with certs type %q", pubKey.Type())
		...
		if err := pubKey.Verify(data, sig); err != nil {
			t.Logf("verify failed with certs type %q", pubKey.Type())
	...
	// If the certs has a lifetime, is it removed when it should be?
	if lifetimeSecs > 0 {
