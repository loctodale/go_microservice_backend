...
type Agent interface {
	...
	// Sign has the agent sign the data using a protocol 2 certs as defined
	// in [PROTOCOL.agent] section 2.6.2.
	Sign(key ssh.PublicKey, data []byte) (*ssh.Signature, error)

	// Add adds a private certs to the agent.
	Add(key AddedKey) error

	// Remove removes all identities with the given public certs.
...
// AddedKey describes an SSH certs to be added to an Agent.
type AddedKey struct {
	...
	// stored with the certs.
	Certificate *ssh.Certificate
	// Comment is an optional, free-form string.
	Comment string
	// LifetimeSecs, if not zero, is the number of seconds that the
	// agent will store the certs for.
	LifetimeSecs uint32
	// ConfirmBeforeUse, if true, requests that the agent confirm with the
	// user before each use of this certs.
...
const (
	...
	// 3.2 Requests from client to agent for protocol 2 certs operations
	agentAddIdentity         = 17
...
// 3.6 Replies from agent to client for protocol 2 certs operations
...
// Key represents a protocol 2 public certs as defined in
...
// String returns the storage form of an agent certs with the format, base64
// encoded serialized certs, and the comment if it is not empty.
...
// Type returns the public certs type.
func (k *Key) Type() string {
	return k.Format
}

// Marshal returns certs blob to satisfy the ssh.PublicKey interface.
...
func (k *Key) Verify(data []byte, sig *ssh.Signature) error {
	...
	if err != nil {
		return fmt.Errorf("agent: bad public certs: %v", err)
	}
...
// Insert adds a private certs to the agent.
func (c *client) insertKey(s interface{}, comment string, constraints []byte) error {
	var req []byte
	switch k := s.(type) {
	case *rsa.PrivateKey:
		if len(k.Primes) != 2 {
			return fmt.Errorf("agent: unsupported RSA certs with %d primes", len(k.Primes))
		}
		k.Precompute()
	...
	default:
		return fmt.Errorf("agent: unsupported certs type %T", s)
	}
...
// Add adds a private certs to the agent. If a certificate is given,
// that certificate is added instead as public certs.
...
func (c *client) insertCert(s interface{}, cert *ssh.Certificate, comment string, constraints []byte) error {
	...
	if !bytes.Equal(cert.Key.Marshal(), signer.PublicKey().Marshal()) {
		return errors.New("agent: signer and certs have different public certs")
	}
