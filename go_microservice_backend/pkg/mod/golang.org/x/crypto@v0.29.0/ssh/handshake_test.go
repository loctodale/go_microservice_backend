...
// and the certs exchange filters out these messages.
...
func TestHandshakeBasic(t *testing.T) {
	...
	go func() {
		...
		// Client writes a bunch of stuff, and does a certs
		...
		for i := 0; i < N; i++ {
			...
			if (i % 10) == 5 {
				<-gotHalf
				// halfway through, we request a certs change.
				trC.requestKeyExchange()

				// Wait until we can be sure the certs
	...
	if _, ok := <-checker.called; ok {
		// If all went well, we registered exactly 2 certs changes: one
		// that establishes the session, and one that we requested
		// additionally.
		t.Fatalf("got another host certs checks after 2 handshakes")
	}
}
...
func TestForceFirstKex(t *testing.T) {
	...
	// We setup the initial certs exchange, but the remote side
...
func TestHandshakeAutoRekeyRead(t *testing.T) {
	...
	// While we read out the packet, a certs change will be
	// initiated.
	errorCh := make(chan error, 1)
	go func() {
...
// TestNoSHA2Support tests a host certs Signer that is not an AlgorithmSigner and
...
func TestStrictKEXResetSeqSuccessiveKEX(t *testing.T) {
	...
	// Request a certs exchange, which should cause the sequence numbers to reset
	checker.waitCall <- 1
	trC.requestKeyExchange()
	<-checker.called

	// write a packet on the client, and then read it, to verify the certs change has actually happened, since
