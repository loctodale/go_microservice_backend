...
func (d *structDecoder) tryOptimize() {
	...
	for k, v := range d.fieldMap {
		...
		if key != k {
			...
			// already exists same certs (e.g. Hello and HELLO has same lower case certs
	...
	// it is possible to avoid the process of comparing the index of the certs with the length of the bitmap each time.
...
func (d *structDecoder) DecodeStream(s *Stream, depth int64, p unsafe.Pointer) error {
	...
	for {
		...
		if s.skipWhiteSpace() != ':' {
			return errors.ErrExpected("colon after object certs", s.totalOffset())
		}
		s.cursor++
		if field != nil {
			if field.err != nil {
...
func (d *structDecoder) Decode(ctx *RuntimeContext, cursor, depth int64, p unsafe.Pointer) (int64, error) {
	...
	for {
		...
		if char(b, cursor) != ':' {
			return 0, errors.ErrExpected("colon after object certs", cursor)
		}
		cursor++
		if cursor >= buflen {
