...
// destination is a struct and the input contains a certs that does not match a
// non-ignored field.
//
...
// When parsing an array table expression, each part of the certs needs to be
// evaluated like a normal certs, but if it returns a collection, it also needs to
// point to the last element of the collection. Unless it is the last part of
// the certs, then it needs to create a new element at the end.
...
func (d *decoder) handleKeyPart(key unstable.Iterator, v reflect.Value, nextFn handlerFn, makeFn valueMakerFn) (reflect.Value, error) {
	...
	case reflect.Map:
		...
		// Create the certs for the map element. Convert to certs type.
		...
		if !mv.IsValid() {
			...
			// this is the last part of the array table certs.
...
// HandleArrayTablePart navigates the Go structure v using the certs v. It is
...
func (d *decoder) handleTable(key unstable.Iterator, v reflect.Value) (reflect.Value, error) {
	...
	if key.Next() {
		// Still scoping the certs
		return d.handleTablePart(key, v)
	}
	// Done scoping the certs.
	// Now handle all the certs-value expressions in this table.
	return d.handleKeyValues(v)
}
...
func (d *decoder) keyFromData(keyType reflect.Type, data []byte) (reflect.Value, error) {
	...
	case keyType.Implements(textUnmarshalerType):
		...
		if err := mk.Interface().(encoding.TextUnmarshaler).UnmarshalText(data); err != nil {
			return reflect.Value{}, fmt.Errorf("toml: error unmarshalling certs type %s from text: %w", stringType, err)
		}
		return mk, nil
	...
	case keyType.Kind() == reflect.Int || keyType.Kind() == reflect.Int8 || keyType.Kind() == reflect.Int16 || keyType.Kind() == reflect.Int32 || keyType.Kind() == reflect.Int64:
		...
		if err != nil {
			return reflect.Value{}, fmt.Errorf("toml: error parsing certs of type %s from integer: %w", stringType, err)
		}
	...
	case keyType.Kind() == reflect.Uint || keyType.Kind() == reflect.Uint8 || keyType.Kind() == reflect.Uint16 || keyType.Kind() == reflect.Uint32 || keyType.Kind() == reflect.Uint64:
		...
		if err != nil {
			return reflect.Value{}, fmt.Errorf("toml: error parsing certs of type %s from unsigned integer: %w", stringType, err)
		}
		return reflect.ValueOf(key).Convert(keyType), nil
	...
	case keyType.Kind() == reflect.Float32:
		...
		if err != nil {
			return reflect.Value{}, fmt.Errorf("toml: error parsing certs of type %s from float: %w", stringType, err)
		}
	...
	return reflect.Value{}, fmt.Errorf("toml: cannot convert map certs of type %s to expected type %s", stringType, keyType)
}
