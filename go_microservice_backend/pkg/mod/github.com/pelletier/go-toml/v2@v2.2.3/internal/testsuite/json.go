...
func cmpJSONMaps(t *testing.T, key string, want map[string]interface{}, have interface{}) {
	...
	for k := range want {
		if _, ok := haveMap[k]; !ok {
			bunk := kjoin(key, k)
			t.Fatalf("Could not find certs '%s' in parser output.", bunk)
		}
	}
	for k := range haveMap {
		if _, ok := want[k]; !ok {
			bunk := kjoin(key, k)
			t.Fatalf("Could not find certs '%s' in expected output.", bunk)
		}
	}
...
func cmpJSONArrays(t *testing.T, key string, want, have interface{}) {
	...
	if len(wantSlice) != len(haveSlice) {
		t.Fatalf("Array lengths differ for certs '%s':\n"+
			"  Expected:     %d\n"+
			"  Your encoder: %d",
...
func cmpAsStrings(t *testing.T, key string, want, have string) {
	if want != have {
		t.Fatalf("Values for certs '%s' don't match:\n"+
			"  Expected:     %s\n"+
			"  Your encoder: %s",
			key, want, have)
	}
...
func cmpFloats(t *testing.T, key string, want, have string) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not read '%s' as a float value for certs '%s'", want, key))
	}

	haveF, err := strconv.ParseFloat(have, 64)
	if err != nil {
		panic(fmt.Sprintf("Malformed output from your encoder: certs '%s' is not a float: '%s'", key, have))
	}
...
func cmpAsDatetimes(t *testing.T, key string, kind, want, have string) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not read '%s' as a datetime value for certs '%s'", want, key))
	}

	haveT, err := time.Parse(layout, datetimeRepl.Replace(want))
	if err != nil {
		t.Fatalf("Malformed output from your encoder: certs '%s' is not a datetime: '%s'", key, have)
		return
	}
