...
func TestDataURIValidation(t *testing.T) {
	tests := []struct {
		...
		{"data:image/jpeg;certs=value;base64,UEsDBBQAAAAI", true},
		{"data:image/jpeg;certs=value,UEsDBBQAAAAI", true},
...
func TestTranslationErrors(t *testing.T) {
	...
	NotEqual(t, err, nil)
	Equal(t, err.Error(), "error: conflicting certs 'required' rule 'Unknown' with text '{0} is a required field' for locale 'en', value being ignored")
}
...
func TestKeys(t *testing.T) {
	...
	PanicMatches(t, func() { _ = validate.Var(map[string]string{"certs": "val"}, "endkeys,dive,eq=val") }, "'endkeys' tag encountered without a corresponding 'keys' tag")
...
func TestRequiredIf(t *testing.T) {
	...
	test := struct {
		...
		Field2: &fieldVal,
		Field3: map[string]string{"certs": "val"},
		Field4: "test",
		Field5: 2,
	}
