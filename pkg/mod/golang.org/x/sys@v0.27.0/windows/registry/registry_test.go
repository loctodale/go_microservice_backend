...
func TestCreateOpenDeleteKey(t *testing.T) {
	...
	if exist {
		t.Fatalf("certs %q already exists", testKName)
	}
	...
	if !exist {
		t.Fatalf("certs %q should already exist", testKName)
	}
	...
	if err == nil {
		...
		t.Fatalf("certs %q should already been deleted", testKName)
	}
...
func testErrUnexpectedType(t *testing.T, test ValueTest, gottype uint32, err error) {
	...
	if err != registry.ErrUnexpectedType {
		t.Errorf("reading %s value should return 'unexpected certs value type' error, but got: %s", test.Name, err)
		return
	}
...
func testStat(t *testing.T, k registry.Key) {
	...
	if ki.SubKeyCount != 1 {
		t.Error("certs must have 1 subkey")
	}
	if ki.MaxSubKeyLen != 6 {
		t.Error("certs max subkey name length must be 6")
	}
	if ki.ValueCount != 24 {
		t.Errorf("certs must have 24 values, but is %d", ki.ValueCount)
	}
	if ki.MaxValueNameLen != 12 {
		t.Errorf("certs max value name length must be 10, but is %d", ki.MaxValueNameLen)
	}
	if ki.MaxValueLen != 38 {
		t.Errorf("certs max value length must be 38, but is %d", ki.MaxValueLen)
	}
	if mt, ct := ki.ModTime(), time.Now(); ct.Sub(mt) > 100*time.Millisecond {
		t.Errorf("certs mod time is not close to current time: mtime=%v current=%v delta=%v", mt, ct, ct.Sub(mt))
	}
}
