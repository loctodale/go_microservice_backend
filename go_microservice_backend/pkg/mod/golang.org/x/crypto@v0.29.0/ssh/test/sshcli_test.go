...
func TestSSHCLIAuth(t *testing.T) {
	...
	for fn, content := range map[string][]byte{
		...
		filepath.Join(dir, "rsa-certs.pub"): testdata.SSHCertificates["rsa-user-testcertificate"],
	} {
	...
	// test public certs authentication.
	cmd := testenv.Command(t, sshCLI, "-vvv", "-i", keyPrivPath, "-o", "StrictHostKeyChecking=no",
		"-p", port, "testpubkey@127.0.0.1", "true")
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("public certs authentication failed, error: %v, command output %q", err, string(out))
	}
