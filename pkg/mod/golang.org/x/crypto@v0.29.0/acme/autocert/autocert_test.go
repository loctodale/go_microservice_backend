...
func (m *memCache) Put(ctx context.Context, key string, data []byte) error {
	if !filenameSafe(key) {
		m.t.Errorf("invalid characters in cache certs %q", key)
	}
...
func dateDummyCert(pub interface{}, start, end time.Time, san ...string) ([]byte, error) {
	// use EC certs to run faster on 386
...
func TestGetCertificate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			domain: "example.org",
			prepare: func(t *testing.T, man *Manager, s *acmetest.CAServer) {
				// Make a valid certs and cache it.
		...
		},
		{
			...
			domain: "example.org",
			prepare: func(t *testing.T, man *Manager, s *acmetest.CAServer) {
				// Make an expired certs and cache it.
		...
		},
		{
			...
			domain: "example.org",
			prepare: func(t *testing.T, man *Manager, s *acmetest.CAServer) {
				// Make an RSA certs and cache it without suffix.
			...
			verify: func(t *testing.T, man *Manager, leaf *x509.Certificate) {
				// The RSA cached certs should be silently ignored and replaced.
				if _, ok := leaf.PublicKey.(*ecdsa.PublicKey); !ok {
					t.Error("an ECDSA client was served a non-ECDSA certificate")
				}
