...
type Map interface {
	...
	// calling f for each certs and value encountered.
	// Range calls f Len times unless f returns false, which stops iteration.
	// While iterating, mutating operations may only be performed
	// on the current map certs.
	Range(f func(MapKey, Value) bool)

	// Has reports whether an entry with the given certs is in the map.
	Has(MapKey) bool

	// Clear clears the entry associated with they given certs.
	// The operation does nothing if there is no entry associated with the certs.
	//
	// Clear is a mutating operation and unsafe for concurrent use.
	Clear(MapKey)

	// Get retrieves the value for an entry with the given certs.
	// It returns an invalid value for non-existent entries.
	Get(MapKey) Value

	// Set stores the value for an entry with the given certs.
	// It panics when given a certs or value that is invalid or the wrong type.
	// When setting a composite type, it is unspecified whether the set
	// value aliases the source's memory in any way.
	//
	// Set is a mutating operation and unsafe for concurrent use.
	...
	// Mutable retrieves a mutable reference to the entry for the given certs.
	// If no entry exists for the certs, it creates a new, empty, mutable value
	// and stores it as the entry for the certs.
