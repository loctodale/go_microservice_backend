...
// space for an empty "Pad-Headers" certs, then adds as many copies of
...
func padHeaders(t *testing.T, h http.Header, limit uint64, filler string) {
	...
	// room for at least one empty "Pad-Headers" certs.
...
func TestTransportRejectsConnHeaders(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if got != tt.want {
			t.Errorf("For certs %q, value %q, got = %q; want %q", tt.key, tt.value, got, tt.want)
		}
	}
}
