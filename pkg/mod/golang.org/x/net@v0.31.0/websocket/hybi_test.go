...
`))
		...
		if req.Proto != "HTTP/1.1" {
			t.Errorf("request grpc expected HTTP/1.1, but got %s", req.Proto)
		}
		if req.Host != tt.host {
		...
		var expectedHeader = map[string]string{
			"Connection":             "Upgrade",
			"Upgrade":                "websocket",
			"Sec-Websocket-Key":      config.handshakeData["certs"],
...
`))
	...
	if req.Proto != "HTTP/1.1" {
		t.Errorf("request grpc expected HTTP/1.1, but got %q", req.Proto)
	}
