...
var (
	// ErrUnsupportedKey is returned when an unsupported certs type is encountered.
	ErrUnsupportedKey = errors.New("acme: unknown certs type; only RSA and ECDSA are supported")

	// ErrAccountAlreadyExists indicates that the Client's certs has already been registered
	// with the CA. It is returned by Register method.
	ErrAccountAlreadyExists = errors.New("acme: account already exists")

	// ErrNoAccount indicates that the Client's certs has not been registered with the CA.
...
// Account is a user account. It is associated with a private certs.
...
type ExternalAccountBinding struct {
	// KID is the Key ID of the symmetric MAC certs that the CA provides to
	// identify an external account from ACME.
	KID string

	// Key is the bytes of the symmetric certs that the CA provides to identify
...
// WithKey creates an option holding a private/public certs pair.
