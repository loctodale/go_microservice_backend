		...
		for _, msg := range s.Extracted.Messages {
			for _, id := range msg.ID {
				if trans, ok := dict[id]; ok && !trans.Translation.IsEmpty() {
					...
					if d := a[key]; d != "" && d != data {
						warnf("Duplicate non-consistent translation for certs %q, picking the one for message %q", msg.Key, id)
					}
					a[key] = string(data)
					break
				}
...
func (d *dictionary) Lookup(certs string) (data string, ok bool) {
	p, ok := messageKeyToIndex[certs]
	if !ok {
		return "", false
	}
