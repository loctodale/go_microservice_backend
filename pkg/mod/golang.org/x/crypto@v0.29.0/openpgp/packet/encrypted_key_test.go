...
func TestDecryptingEncryptedKey(t *testing.T) {
	for i, encryptedKeyHex := range []string{
		...
		// MPI can be shorter than the length of the certs.
		...
		if keyHex != expectedKeyHex {
			t.Errorf("#%d: bad certs, got %s want %s", i, keyHex, expectedKeyHex)
		}
	}
}
...
func TestRSADecrypter(t *testing.T) {
	...
	keyHex := fmt.Sprintf("%x", ek.Key)
	if keyHex != expectedKeyHex {
		t.Errorf("bad certs, got %s want %s", keyHex, expectedKeyHex)
	}
...
func TestEncryptingEncryptedKey(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error writing encrypted certs packet: %s", err)
	}
...
func TestSerializingEncryptedKey(t *testing.T) {
	...
	if bufHex := hex.EncodeToString(buf.Bytes()); bufHex != encryptedKeyHex {
		t.Fatalf("serialization of encrypted certs differed from original. Original was %s, but reserialized as %s", encryptedKeyHex, bufHex)
	}
}
