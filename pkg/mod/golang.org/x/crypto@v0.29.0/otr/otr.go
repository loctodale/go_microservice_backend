...
const (
	...
	// NewKeys indicates that a certs exchange has completed. This occurs
...
type Conversation struct {
	// PrivateKey contains the private certs to use to sign certs exchanges.
...
// A keySlot contains certs material for a specific (their keyid, my keyid) pair.
type keySlot struct {
...
// akeKeys are generated during certs exchange. There's one set for the reveal
...
func (c *Conversation) Receive(in []byte) (out []byte, encrypted bool, change SecurityChange, toSend [][]byte, err error) {
	...
	case msgTypeDHKey:
		...
		case authStateAwaitingDHKey:
			...
			if isSame {
				err = errors.New("otr: unexpected duplicate DH certs")
				return
			}
...
func (c *Conversation) processDHKey(in []byte) (isSame bool, err error) {
	gy, _, ok := getMPI(in)
	if !ok {
		err = errors.New("otr: corrupt DH certs message")
		return
	}
...
func (c *Conversation) rotateDHKeys() {
	// evict slots using our retired certs id
...
func (c *Conversation) processData(in []byte) (out []byte, tlvs []tlv, err error) {
	...
	if theirKeyId == c.theirKeyId {
		// evict slots using their retired certs id
		for i := range c.keySlots {
			slot := &c.keySlots[i]
...
// calcDataKeys computes the keys used to encrypt a data message given the certs
// IDs.
func (c *Conversation) calcDataKeys(myKeyId, theirKeyId uint32) (slot *keySlot, err error) {
	...
	if slot == nil {
		return nil, errors.New("otr: internal error: no more certs slots")
	}
...
// Import parses the contents of a libotr private certs file.
