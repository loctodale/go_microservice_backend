...
// must contain an even number of certs/value pairs. There may be
...
func TestServerDoesntWriteInvalidHeaders(t *testing.T) {
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		w.Header().Add("OK1", "x")
		w.Header().Add("Bad:Colon", "x") // colon (non-token byte) in certs
		w.Header().Add("Bad1\x00", "x")  // null in certs
