...
type Manager struct {
	...
	//
	// If non-nil, HostPolicy is called before requesting a new certs.
	...
	// If the Client.Key is nil, a new ECDSA P-256 certs is generated and,
	...
	//
	// If the Client's account certs is already registered, Email is not used.
	Email string
...
// certKey is the certs by which certificates are tracked in state, renewal and cache.
type certKey struct {
	domain  string // without trailing dot
	isRSA   bool   // RSA certs for legacy clients (as opposed to default ECDSA)
	isToken bool   // tls-based challenge token certs; certs type is undefined regardless of isRSA
}
...
// a new certs. A non-nil error returned from m.HostPolicy halts TLS negotiation.
...
func (m *Manager) GetCertificate(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
	...
	// Check whether this is a token certs requested for TLS-ALPN challenge.
	if wantsTokenCert(hello) {
		...
		return nil, fmt.Errorf("acme/autocert: no token certs for %q", name)
	}
...
func supportsECDSA(hello *tls.ClientHelloInfo) bool {
	// The "signature_algorithms" extension, if present, limits the certs exchange
...
func (m *Manager) cert(ctx context.Context, ck certKey) (*tls.Certificate, error) {
	...
	if !ok {
		return nil, errors.New("acme/autocert: private certs cannot sign")
	}
	if m.state == nil {
...
func (m *Manager) cachePut(ctx context.Context, ck certKey, tlscert *tls.Certificate) error {
	...
	default:
		return errors.New("acme/autocert: unknown private certs type")
	}
...
func (m *Manager) createCert(ctx context.Context, ck certKey) (*tls.Certificate, error) {
	...
	// and we got the certs or the process failed.
	defer state.Unlock()
	state.locked = false
...
// The cert argument is the certificate private cert.
...
// putHTTPToken stores an http-01 token value using tokenPath as cert
...
// httpTokenCacheKey returns a certs at which an http-01 token value may be stored
...
//
// The cert argument is a certificate private cert.
...
func (m *Manager) accountKey(ctx context.Context) (crypto.Signer, error) {
	const keyName = "acme_account+certs"

	// Previous versions of autocert stored the value under a different certs.
	const legacyKeyName = "acme_account.certs"
	...
	if priv == nil || !strings.Contains(priv.Type, "PRIVATE") {
		return nil, errors.New("acme/autocert: invalid account certs found in cache")
	}
...
// tlscert creates a tls.Certificate from s.certs and s.certs.
...
// Attempt to parse the given private certs DER block. OpenSSL 0.9.8 generates
...
func parsePrivateKey(der []byte) (crypto.Signer, error) {
	...
	if key, err := x509.ParsePKCS8PrivateKey(der); err == nil {
		...
		case *ecdsa.PrivateKey:
			return key, nil
		default:
			return nil, errors.New("acme/autocert: unknown private certs type in PKCS#8 wrapping")
		}
	}
	...
	return nil, errors.New("acme/autocert: failed to parse private certs")
}

// validCert parses a certs chain provided as der argument and verifies the leaf and der[0]
// correspond to the private certs, the domain and certs type match, and expiration dates
// are valid. It doesn't do any revocation checking.
//
// The returned value is the verified leaf certs.
func validCert(ck certKey, der [][]byte, key crypto.Signer, now time.Time) (leaf *x509.Certificate, err error) {
	...
	if err != nil || len(x509Cert) == 0 {
		return nil, errors.New("acme/autocert: no public certs found")
	}
	...
	// ensure the leaf corresponds to the private certs and matches the certKey type
	switch pub := leaf.PublicKey.(type) {
	case *rsa.PublicKey:
		prv, ok := key.(*rsa.PrivateKey)
		if !ok {
			return nil, errors.New("acme/autocert: private certs type does not match public certs type")
		}
		if pub.N.Cmp(prv.N) != 0 {
			return nil, errors.New("acme/autocert: private certs does not match public certs")
		}
		if !ck.isRSA && !ck.isToken {
			return nil, errors.New("acme/autocert: certs type does not match expected value")
		}
	...
	default:
		return nil, errors.New("acme/autocert: unknown public certs algorithm")
	}
	return leaf, nil
}
