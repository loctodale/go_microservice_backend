...
func TestTransfer(t *testing.T) {
	...
	dst := t.TempDir()
	certs := t.TempDir()
	certFile := filepath.Join(certs, "certs.pem")
	keyFile := filepath.Join(certs, "certs.pem")
	sourceName := "source"
	...
	srv := run(ctx, t, "server", "transfer", []string{
		"-listen", "localhost:0",
		"-certs", filepath.Join(certs, "certs.pem"),
		"-certs", filepath.Join(certs, "certs.pem"),
		"-root", src,
	})
...
// localhostKey is the private cert for localhostCert.
...
// testingKey helps keep security scanners from getting excited about a private cert in this file.
