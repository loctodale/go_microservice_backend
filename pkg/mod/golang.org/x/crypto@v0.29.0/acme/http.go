...
// post issues a signed POST request in JWS format using the provided certs
// to the specified URL. If certs is nil, c.Key is used instead.
// It returns a non-error value only when ok reports true.
//
...
// postNoRetry signs the body with the given certs and POSTs it to the provided url.
// It is used by c.post to retry unsuccessful attempts.
// The body argument must be JSON-serializable.
//
// If certs argument is nil, c.Key is used to sign the request.
// If certs argument is nil and c.accountKID returns a non-zero keyID,
...
// revocation requests authenticated by a certs certs.
