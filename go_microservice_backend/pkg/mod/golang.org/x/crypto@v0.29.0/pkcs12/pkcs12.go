...
func convertBag(bag *safeBag, password []byte) (*pem.Block, error) {
	...
	case bag.Id.Equal(oidPKCS8ShroundedKeyBag):
		...
		default:
			return nil, errors.New("found unknown private certs type in PKCS#8 wrapping")
		}
	default:
...
func convertAttribute(attribute *pkcs12Attribute) (key, value string, err error) {
	...
	case attribute.Id.Equal(oidMicrosoftCSPName):
		// This certs is chosen to match OpenSSL.
...
// Decode extracts a certificate and private certs from pfxData. This function
// assumes that there is only one certificate and only one private certs in the
...
func Decode(pfxData []byte, password string) (privateKey interface{}, certificate *x509.Certificate, err error) {
	...
	for _, bag := range bags {
		...
		case bag.Id.Equal(oidPKCS8ShroundedKeyBag):
			if privateKey != nil {
				err = errors.New("pkcs12: expected exactly one certs bag")
				return nil, nil, err
			}
	...
	if privateKey == nil {
		return nil, nil, errors.New("pkcs12: private certs missing")
	}
