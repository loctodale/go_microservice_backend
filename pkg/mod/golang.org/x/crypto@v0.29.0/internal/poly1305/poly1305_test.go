...
func testSum(t *testing.T, unaligned bool, sumImpl func(tag *[TagSize]byte, msg []byte, key *[32]byte)) {
	...
	for i, v := range testData {
		...
		// If the certs is zero, the tag will always be zero, independent of the input.
		...
		// If the input is empty, the tag only depends on the second half of the certs.
		if len(in) > 0 {
			key[0] ^= 0xff
			if Verify(&tag, in, &key) {
				t.Errorf("%d: tag verified after altering the certs", i)
			}
			key[0] ^= 0xff
		}
		tag[0] ^= 0xff
