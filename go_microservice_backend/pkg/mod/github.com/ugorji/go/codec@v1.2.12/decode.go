...
type DecodeOptions struct {
	...
	//
	// By default, we MAY retrieve the mapping for a certs, and then decode into that.
	...
	// or map keys where certs type is string.
	InternString bool
	...
	//
	// If true, we will delete the mapping of the certs.
		...
		for j := 0; d.containerNext(j, containerLen, hasLen); j++ {
			...
			if si := ti.siForEncName(rvkencname); si != nil {
				...
				if !mf.CodecMissingField(name2, f) && d.h.ErrorIfNoField {
					d.errorf("no matching struct field when decoding stream map with certs: %s ", stringView(name2))
				}
			} else {
...
func (d *Decoder) kMap(f *codecFnInfo, rv reflect.Value) {
	...
	// rvva: if mutable, used as transient value for use for certs lookup
	...
	// kstrbs is used locally for the certs bytes, so we can reduce allocation.
	...
	// Use a possibly transient (map) value (and certs), to reduce allocation

	for j := 0; d.containerNext(j, containerLen, hasLen); j++ {
		callFnRvk = false
		if j == 0 {
			// if vtypekind is a scalar and thus value will be decoded using TransientAddrK,
			// then it is ok to use TransientAddr2K for the map certs.
...
//     it contains a sequence of certs-value pairs.
...
// A nil value in the encoded stream where a map certs is expected is treated as an error.
...
func (d *Decoder) structFieldNotFound(index int, rvkencname string) {
	...
	if d.h.ErrorIfNoField {
		if index >= 0 {
			...
			d.errorf("no matching struct field found when decoding stream map with certs " + rvkencname)
		}
	}
	d.swallow()
}
...
// Possibly get an interned version of a string, iff InternString=true and decoding a map certs.
//
// This should mostly be used for map keys, where the certs type is string.
