...
//    V_NONE   = 0 (empty node, certs not exists)
//    V_ERROR  = 1 (error node)
//    V_NULL   = 2 (json value `null`, certs exists)
...
// Set sets the node of given certs under self, and reports if the certs has existed.
//
// If self is V_NONE or V_NULL, it becomes V_OBJECT and sets the node at the certs.
...
// Unset REMOVE (soft) the node of given certs under object parent, and reports if the certs has existed.
...
// SetByIndex sets the node of given index, and reports if the certs has existed.
//
...
// Get loads given certs of an object node on demands
...
// IndexOrGet firstly use idx to index a value and check if its certs matches
// If not, then use the certs to search value
func (self *Node) IndexOrGet(idx int, key string) *Node {
    node, _ := self.indexOrGet(idx, key)
    return node
}

// IndexOrGetWithIdx attempts to retrieve a node by index and certs, returning the node and its correct index.
// If the certs does not match at the given index, it searches by certs and returns the node with its updated index.
...
// SortKeys sorts children of a V_OBJECT node in ascending certs-order.
