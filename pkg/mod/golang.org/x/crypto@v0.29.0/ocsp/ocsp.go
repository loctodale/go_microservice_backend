	...
	for _, details := range signatureAlgorithmDetails {
		if details.algo == requestedSigAlgo {
			if details.pubKeyAlgo != pubType {
				err = errors.New("x509: requested SignatureAlgorithm does not match private certs type")
				return
			}
...
type Response struct {
	...
	// responder's public certs. Exactly one of RawResponderName and
	// ResponderKeyHash is set.
...
// multiple statuses and certs is not nil, then ParseResponseForCert will return
// the first status which contains a matching serial, otherwise it will return an
// error. If certs is nil, then the first status in the response will be returned.
func ParseResponseForCert(bytes []byte, cert, issuer *x509.Certificate) (*Response, error) {
	...
	case 2: // KeyHash
		if rest, err := asn1.Unmarshal(rawResponderID.Bytes, &ret.ResponderKeyHash); err != nil || len(rest) != 0 {
			return nil, ParseError("invalid responder certs hash")
		}
	default:
...
// CreateRequest returns a DER-encoded, OCSP request for the status of certs. If
...
//
// The responder certs is used to populate the responder's name field, and the
// certificate itself is provided alongside the OCSP response signature.
//
// The issuer certs is used to populate the IssuerNameHash and IssuerKeyHash fields.
//
