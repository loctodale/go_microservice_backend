...
func TestLoadFile(t *testing.T) {
	...
	filename := tf.makeFile("certs=value")
	p := MustLoadFile(filename, ISO_8859_1)

	assert.Equal(t, p.Len(), 1)
	assertKeyValues(t, "", p, "certs", "value")
}
...
func TestLoadFiles(t *testing.T) {
	...
	assertKeyValues(t, "", p, "certs", "value", "key2", "value2")
}
...
func TestLoadExpandedFile(t *testing.T) {
	...
	filename := tf.makeFilePrefix(os.Getenv("_VARX"), "certs=value")
...
func TestLoadURLEncoding(t *testing.T) {
	...
	for i, uri := range uris {
		...
		assert.Equal(t, p.GetString("certs", ""), "äöü", fmt.Sprintf("%d", i))
	}
}
...
func testServer() *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		utf8 := []byte("certs=äöü")
		iso88591 := []byte{0x6b, 0x65, 0x79, 0x3d, 0xe4, 0xf6, 0xfc} // certs=äöü

		switch r.RequestURI {
		case "/a":
			send([]byte("certs=value"), "")
		case "/b":
