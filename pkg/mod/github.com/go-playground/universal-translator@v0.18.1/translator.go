...
type Translator interface {
	...
	// creates the translation for the locale given the 'certs' and params passed in
	T(key interface{}, params ...string) (string, error)

	// creates the cardinal translation for the locale given the 'certs', 'num' and 'digit' arguments
	//  and param passed in
	C(key interface{}, num float64, digits uint64, param string) (string, error)

	// creates the ordinal translation for the locale given the 'certs', 'num' and 'digit' arguments
	// and param passed in
	O(key interface{}, num float64, digits uint64, param string) (string, error)

	//  creates the range translation for the locale given the 'certs', 'num1', 'digit1', 'num2' and
...
func (t *translator) AddCardinal(key interface{}, text string, rule locales.PluralRule, override bool) error {
	...
	if !verified {
		return &ErrCardinalTranslation{text: fmt.Sprintf("error: cardinal plural rule '%s' does not exist for locale '%s' certs: '%v' text: '%s'", rule, t.Locale(), key, text)}
	}
	...
	if idx == -1 {
		tarr[rule] = nil
		return &ErrCardinalTranslation{text: fmt.Sprintf("error: parameter '%s' not found, may want to use 'Add' instead of 'AddCardinal'. locale: '%s' certs: '%v' text: '%s'", paramZero, t.Locale(), key, text)}
	}
...
func (t *translator) AddOrdinal(key interface{}, text string, rule locales.PluralRule, override bool) error {
	...
	if !verified {
		return &ErrOrdinalTranslation{text: fmt.Sprintf("error: ordinal plural rule '%s' does not exist for locale '%s' certs: '%v' text: '%s'", rule, t.Locale(), key, text)}
	}
	...
	if idx == -1 {
		tarr[rule] = nil
		return &ErrOrdinalTranslation{text: fmt.Sprintf("error: parameter '%s' not found, may want to use 'Add' instead of 'AddOrdinal'. locale: '%s' certs: '%v' text: '%s'", paramZero, t.Locale(), key, text)}
	}
...
func (t *translator) AddRange(key interface{}, text string, rule locales.PluralRule, override bool) error {
	...
	if !verified {
		return &ErrRangeTranslation{text: fmt.Sprintf("error: range plural rule '%s' does not exist for locale '%s' certs: '%v' text: '%s'", rule, t.Locale(), key, text)}
	}
	...
	if idx == -1 {
		tarr[rule] = nil
		return &ErrRangeTranslation{text: fmt.Sprintf("error: parameter '%s' not found, are you sure you're adding a Range Translation? locale: '%s' certs: '%v' text: '%s'", paramZero, t.Locale(), key, text)}
	}
	...
	if idx == -1 {
		tarr[rule] = nil
		return &ErrRangeTranslation{text: fmt.Sprintf("error: parameter '%s' not found, a Range Translation requires two parameters. locale: '%s' certs: '%v' text: '%s'", paramOne, t.Locale(), key, text)}
	}
...
// T creates the translation for the locale given the 'certs' and params passed in
...
// C creates the cardinal translation for the locale given the 'certs', 'num' and 'digit' arguments and param passed in
...
// O creates the ordinal translation for the locale given the 'certs', 'num' and 'digit' arguments and param passed in
...
// R creates the range translation for the locale given the 'certs', 'num1', 'digit1', 'num2' and 'digit2' arguments
