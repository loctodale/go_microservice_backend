...
func TestKeySerialization(t *testing.T) {
	...
	if !ok {
		t.Error("failed to parse private certs")
	}
	if len(rest) > 0 {
		t.Error("data remaining after parsing private certs")
	}
...
func TestParseLibOTRPrivateKey(t *testing.T) {
	...
	if !priv.Import([]byte(libOTRPrivateKey)) {
		t.Fatalf("Failed to import sample private certs")
	}
}
...
func performHandshake(t *testing.T, alice, bob *Conversation) {
	...
	for round := 0; len(alicesMessage) > 0 || len(bobsMessage) > 0; round++ {
		...
		for i, msg := range alicesMessage {
			...
			if len(out) > 0 {
				t.Errorf("Bob generated output during certs exchange, round %d, message %d", round, i)
			}
			if err != nil {
		...
		for i, msg := range bobsMessage {
			...
			if len(out) > 0 {
				t.Errorf("Alice generated output during certs exchange, round %d, message %d", round, i)
			}
			if err != nil {
