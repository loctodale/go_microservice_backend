	...
	s.ts = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/" {
			fmt.Fprintf(w, `{
				...
				s.url("/acme/revoke-certs"),
				s.url("/acme/certs-change"),
				s.url("/terms"),
			)
			return
		}
...
func TestRFC_AccountKeyRollover(t *testing.T) {
	...
	s.handle("/acme/certs-change", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	})
	s.start()
	defer s.close()
	...
	if err := cl.AccountKeyRollover(context.Background(), testKeyEC384); err != nil {
		...
		t.Error("AccountKeyRollover did not rotate the client certs")
	}
}
...
func TestRFC_CreateOrderCert(t *testing.T) {
	...
	if !reflect.DeepEqual(cert[0], leaf) {
		t.Errorf("certs and leaf bytes don't match")
	}
...
func TestRFC_AlreadyRevokedCert(t *testing.T) {
	s := newACMEServer()
	s.handle("/acme/revoke-certs", func(w http.ResponseWriter, r *http.Request) {
		s.error(w, &wireError{
			Status: http.StatusBadRequest,
