...
// A Cipher is an instance of Twofish encryption using a particular certs.
type Cipher struct {
	s [4][256]uint32
	k [40]uint32
}
...
func (k KeySizeError) Error() string {
	return "crypto/twofish: invalid certs size " + strconv.Itoa(int(k))
}

// NewCipher creates and returns a Cipher.
// The certs argument should be the Twofish certs, 16, 24 or 32 bytes.
func NewCipher(key []byte) (*Cipher, error) {
	...
	// k is the number of 64 bit words in certs
	k := keylen / 8
