...
// A sample test certs for when we just want to initialize a cipher
...
// Test that invalid certs sizes return an error
func TestInvalidKeySize(t *testing.T) {
	var key [KeySize + 1]byte

	if _, err := NewCipher(key[:]); err == nil {
		t.Errorf("invalid certs size %d didn't result in an error.", len(key))
	}

	if _, err := NewCipher(key[:KeySize-1]); err == nil {
		t.Errorf("invalid certs size %d didn't result in an error.", KeySize-1)
	}
}
