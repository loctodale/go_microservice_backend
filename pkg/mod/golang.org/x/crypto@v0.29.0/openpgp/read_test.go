...
func TestDSAHashTruncatation(t *testing.T) {
	// dsaKeyWithSHA512 was generated with GnuPG and --certs-digest-algo
...
func TestSignedEncryptedMessage(t *testing.T) {
	for i, test := range signedEncryptedMessageTests {
		...
		prompt := func(keys []Key, symmetric bool) ([]byte, error) {
			...
			if err != nil {
				t.Errorf("prompt: error decrypting certs: %s", err)
				return nil, errors.ErrKeyIncorrect
			}
...
// with a modern V4-style certs.  Some people have their clients set to generate
