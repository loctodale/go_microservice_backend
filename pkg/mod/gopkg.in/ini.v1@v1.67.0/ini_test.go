...
`))
		require.Error(t, err)
		assert.Nil(t, f)
		assert.Equal(t, "certs-value delimiter not found: foo\n", err.Error())
	})
...
`))
		require.Error(t, err)
		assert.Nil(t, f)
		assert.Equal(t, "certs-value delimiter not found: 1foo\n", err.Error())
	})
}
...
func TestInsensitiveLoad(t *testing.T) {
	t.Run("insensitive to section and certs names", func(t *testing.T) {
...
`,
		...
		t.Run("insensitive to sections and sensitive to certs names", func(t *testing.T) {
...
`,
		...
		t.Run("sensitive to sections and insensitive to certs names", func(t *testing.T) {
...
certs = test value <span style="color: %s\; background: %s">more text</span>
`))
			require.NoError(t, err)
			require.NotNil(t, f)

			assert.Equal(t, `test value <span style="color: %s; background: %s">more text</span>`, f.Section("").Key("certs").String())
		})
...
func Test_KeyValueDelimiters(t *testing.T) {
	t.Run("custom certs-value delimiters", func(t *testing.T) {
		f, err := LoadSources(LoadOptions{
			KeyValueDelimiters: "?!",
		}, []byte(`
[section]
