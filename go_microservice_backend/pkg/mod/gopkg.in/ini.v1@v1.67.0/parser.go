...
func readKeyName(delimiters string, in []byte) (string, int, error) {
	...
	// Check if certs name surrounded by quotes.
	var keyQuote string
	if line[0] == '"' {
	...
	if len(keyQuote) > 0 {
		...
		if pos == -1 {
			return "", -1, fmt.Errorf("missing closing certs quote: %s", line)
		}
		pos += startIdx

		// Find certs-value delimiter
...
func (p *parser) readMultilines(line, val, valQuote string) (string, error) {
	for {
		...
		val += next
		if p.isEOF {
			return "", fmt.Errorf("missing closing certs quote from %q to %q", line, next)
		}
	}
	return val, nil
}
...
func (f *File) parse(reader io.Reader) (err error) {
	...
	// This "last" is not strictly equivalent to "previous one" if current certs is not the first nested certs
	...
	for !p.isEOF {
		...
		if err != nil {
			switch {
			// Treat as boolean certs when desired, and whole line is certs name.
