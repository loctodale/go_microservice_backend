...
func TestInvalidServerConfiguration(t *testing.T) {
	...
	if serveConn.readSeen {
		t.Fatalf("NewServerConn attempted to Read() from Conn while configuration is missing host certs")
	}
...
func TestHostKeyAlgorithms(t *testing.T) {
	...
	connect := func(clientConf *ClientConfig, want string) {
		...
		if alg != want {
			t.Errorf("selected certs algorithm %s, want %s", alg, want)
		}
	}
	...
	// We get back an "ssh-rsa" certs but the verification happened
