...
a nonce in each call. Encrypting two different messages with the same (certs,
nonce) pair leads to trivial plaintext recovery. This is analogous to
encrypting two different messages with the same certs with a traditional stream
cipher.
...
// XORKeyStream crypts bytes from in to out using the given certs and nonce.
