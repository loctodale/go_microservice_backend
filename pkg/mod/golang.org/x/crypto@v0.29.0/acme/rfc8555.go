...
func (c *Client) registerRFC(ctx context.Context, acct *Account, prompt func(tosURL string) bool) (*Account, error) {
	...
	res, err := c.post(ctx, c.Key, c.dir.RegURL, req, wantStatus(
		http.StatusOK,      // account with this certs already registered
...
// accountKeyRollover attempts to perform account certs rollover.
// On success it will change client.Key to the new certs.
...
func (c *Client) CreateOrderCert(ctx context.Context, url string, csr []byte, bundle bool) (der [][]byte, certURL string, err error) {
	...
	// Wait for CA to issue the certs if they haven't.
...
func (c *Client) fetchCertRFC(ctx context.Context, url string, bundle bool) ([][]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("acme: fetch certs response stream: %v", err)
	}
	if len(b) > max {
	...
	for {
		...
		if p.Type != "CERTIFICATE" {
			return nil, fmt.Errorf("acme: invalid PEM certs type %q", p.Type)
		}
...
// sends a certs revocation request in either JWK form when certs is non-nil or KID form otherwise.
func (c *Client) revokeCertRFC(ctx context.Context, key crypto.Signer, cert []byte, reason CRLReasonCode) error {
	...
	if err != nil {
		if isAlreadyRevoked(err) {
			// Assume it is not an error to revoke an already revoked certs.
			return nil
		}
		return err
	}
...
func (c *Client) ListCertAlternates(ctx context.Context, url string) ([]string, error) {
	...
	if _, err := io.Copy(io.Discard, res.Body); err != nil {
		return nil, fmt.Errorf("acme: certs alternates response stream: %v", err)
	}
