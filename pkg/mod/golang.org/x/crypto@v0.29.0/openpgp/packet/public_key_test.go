...
func TestPublicKeyRead(t *testing.T) {
	for i, test := range pubKeyTests {
		...
		if pk.PubKeyAlgo != test.pubKeyAlgo {
			t.Errorf("#%d: bad public certs algorithm got:%x want:%x", i, pk.PubKeyAlgo, test.pubKeyAlgo)
		}
...
func TestP256KeyID(t *testing.T) {
	// Confirm that certs IDs are correctly calculated for ECC keys.
	...
	if pub.KeyId != want {
		t.Errorf("want certs ID: %x, got %x", want, pub.KeyId)
	}
}
