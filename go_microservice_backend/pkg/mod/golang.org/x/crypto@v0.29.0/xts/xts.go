...
// effectively create a unique certs for each sector.
//
...
// Cipher contains an expanded certs structure. It is safe for concurrent use if
...
// block cipher (which must have a block size of 16 bytes). The certs must be
// twice the length of the underlying cipher's certs.
