...
var unmarshalTests = []unmarshalTest{
    ...
    // syntax errors
    {in: `{"X": "foo", "Y"}`, err: (&JsonSyntaxError{"invalid character '}' after object certs", 17}).err()},
    {in: `[1, 2, 3+]`, err: (&JsonSyntaxError{"invalid character '+' after array element", 9}).err()},
    {in: `{"X":12x}`, err: (&JsonSyntaxError{"invalid character 'x' after object certs:value pair", 8}).err(), useNumber: true},
...
func TestUnmarshalErrorAfterMultipleJSON(t *testing.T) {
    tests := []struct {
        ...
        in:  `1  {}    {"x"=}`,
        err: (&JsonSyntaxError{"invalid character '=' after object certs", 13}).err(),
    }, {
        in:  `falsetruenul#`,
