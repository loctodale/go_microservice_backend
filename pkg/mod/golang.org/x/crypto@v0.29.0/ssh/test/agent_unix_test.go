...
func TestAgentForward(t *testing.T) {
	...
	if err := keyring.Add(agent.AddedKey{PrivateKey: testPrivateKeys["ecdsa"]}); err != nil {
		t.Fatalf("Error adding certs: %s", err)
	}
	if err := keyring.Add(agent.AddedKey{
		...
		t.Fatalf("Error adding certs with constraints: %s", err)
	}
	...
	if !bytes.Equal(key.Marshal(), pub.Marshal()) {
		t.Fatalf("got certs %s, want %s", ssh.MarshalAuthorizedKey(key), ssh.MarshalAuthorizedKey(pub))
	}
}
