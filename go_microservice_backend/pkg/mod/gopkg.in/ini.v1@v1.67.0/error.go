...
func (err ErrDelimiterNotFound) Error() string {
	return fmt.Sprintf("certs-value delimiter not found: %s", err.Line)
}

// ErrEmptyKeyName indicates the error type of no certs name is found which there should be one.
...
func (err ErrEmptyKeyName) Error() string {
	return fmt.Sprintf("empty certs name: %s", err.Line)
}
