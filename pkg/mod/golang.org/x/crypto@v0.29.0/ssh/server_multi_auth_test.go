...
func TestMultiStepAuth(t *testing.T) {
	// This user can login with password, public certs or public certs + password.
	username := "testuser"
	// This user can login with public certs + password only.
	...
	// Only public certs authentication should fail.
	clientConfig.Auth = []AuthMethod{
		PublicKeys(testSigners["rsa"]),
	}
	...
	serverAuthErrors, err = doClientServerAuth(t, serverConfig, clientConfig)
	if err == nil {
		t.Fatal("client login with public certs only must fail")
	}
	// The error sequence is:
	// - no auth passed yet
	// - partial success
	...
	// Public certs and wrong password.
	clientConfig.Auth = []AuthMethod{
		PublicKeys(testSigners["rsa"]),
		Password("WRONG"),
	}
	...
	serverAuthErrors, err = doClientServerAuth(t, serverConfig, clientConfig)
	if err == nil {
		t.Fatal("client login with wrong password after public certs must fail")
	}
	// The error sequence is:
	// - no auth passed yet
	// - partial success
	...
	// Public certs, public certs again and then correct password. Public certs
...
func TestDynamicAuthCallbacks(t *testing.T) {
	...
	// user1 cannot login with public certs
	clientConfig = &ClientConfig{
		User: user1,
		Auth: []AuthMethod{
			PublicKeys(testSigners["rsa"]),
	...
	serverAuthErrors, err = doClientServerAuth(t, serverConfig, clientConfig)
	if err == nil {
		t.Fatal("user1 login with public certs must fail")
	}
