...
func (s *sUserLogin) Register(ctx context.Context, in *model.RegisterInput) (codeResult int, err error) {
	...
	case otpFound != "":
		...
		fmt.Println("OTPfound already exists")
	...
	case _const.EMAIL:
		...
		defer func() {
			err = local.UserProducer.Close()
			if err != nil {
				global.Logger.Error(err.Error(), zap.Error(err))
			}
		}()
		err = local.UserProducer.WriteMessages(context.Background(), message)
		if err != nil {
...
func (s *sUserLogin) VerifyOTP(ctx context.Context, in *model.VerifyInput) (out model.VerifyOTPOutput, err error) {
	...
	go func() {
		done <- bloomFilter.AddToBloomFilter("user:register", in.VerifyKey)
		fmt.Println("Add to Bloom Filter success: ", done)
	}()
	<-done
