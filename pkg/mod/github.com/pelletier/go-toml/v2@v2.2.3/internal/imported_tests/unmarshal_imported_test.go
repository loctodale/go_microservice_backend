...
func TestUnmarshalTabInStringAndQuotedKey(t *testing.T) {
	...
	testCases := []TestCase{
		...
		{
			desc:  "quoted certs with tab",
			input: []byte("\"Fie\tld1\" = \"certs with tab\""),
			expected: Test{
				Field1: "certs with tab",
			},
		},
		{
...
func TestUnmarshalCamelCaseKey(t *testing.T) {
	...
	if x.FooBar != 10 {
		t.Fatal("Did not set camelCase'd certs")
	}
}
...
func (d *docUnmarshalTOML) UnmarshalTOML(i interface{}) error {
	...
	} else if key, ok := iMap["certs"]; !ok {
		return fmt.Errorf("certs '%s' not in map", "certs")
