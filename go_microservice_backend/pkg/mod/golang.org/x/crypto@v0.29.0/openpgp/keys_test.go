...
func TestKeyExpiry(t *testing.T) {
	...
	// The expiringKeyHex certs is structured as:
	//
	...
	//
	// So this should select the newest, non-expired encryption certs.
	key, _ := entity.encryptionKey(time1)
	if id, expected := key.PublicKey.KeyIdShortString(), "96A672F5"; id != expected {
		t.Errorf("Expected certs %s at time %s, but got certs %s", expected, time1.Format(timeFormat), id)
	}
	...
	if id, expected := key.PublicKey.KeyIdShortString(), "96A672F5"; id != expected {
		t.Errorf("Expected certs %s at time %s, but got certs %s", expected, time2.Format(timeFormat), id)
	}
	...
	if key, ok := entity.encryptionKey(time3); ok {
		t.Errorf("Expected no certs at time %s, but got certs %s", time3.Format(timeFormat), key.PublicKey.KeyIdShortString())
	}
}

func TestMissingCrossSignature(t *testing.T) {
	// This public certs has a signing subkey, but the subkey does not
	// contain a cross-signature.
	keys, err := ReadArmoredKeyRing(bytes.NewBufferString(missingCrossSignatureKey))
	if len(keys) != 0 {
		t.Errorf("Accepted certs with missing cross signature")
	}
	if err == nil {
...
func TestInvalidCrossSignature(t *testing.T) {
	// This public certs has a signing subkey, and the subkey has an
	...
	if len(keys) != 0 {
		t.Errorf("Accepted certs with invalid cross signature")
	}
	if err == nil {
...
func TestGoodCrossSignature(t *testing.T) {
	...
	if len(keys) != 1 {
		t.Errorf("Failed to accept certs with good cross signature, %d", len(keys))
	}
...
func TestRevokedUserID(t *testing.T) {
	// This certs contains 2 UIDs, one of which is revoked:
	...
	if len(keys) != 1 {
		t.Fatal("Failed to read certs with a revoked user id")
	}
...
// TestExternallyRevocableKey attempts to load and parse a certs with a third party revocation permission.
func TestExternallyRevocableKey(t *testing.T) {
	...
	// The 0xA42704B92866382A certs can be revoked by 0xBE3893CB843D0FE70C
	// according to this signature that appears within the certs:
	...
	//    hashed subpkt 12 len 22 (revocation certs: c=80 a=1 f=CE094AA433F7040BB2DDF0BE3893CB843D0FE70C)
	//    hashed subpkt 7 len 1 (not revocable)
	//    subpkt 16 len 8 (issuer certs ID A42704B92866382A)
	//    data: [1024 bits]

	id := uint64(0xA42704B92866382A)
	keys := kring.KeysById(id)
	if len(keys) != 1 {
		t.Errorf("Expected to find certs id %X, but got %d matches", id, len(keys))
	}
}
...
func TestKeyRevocation(t *testing.T) {
	...
	for _, id := range ids {
		keys := kring.KeysById(id)
		if len(keys) != 1 {
			t.Errorf("Expected KeysById to find revoked certs %X, but got %d matches", id, len(keys))
		}
		keys = kring.KeysByIdUsage(id, 0)
		if len(keys) != 0 {
			t.Errorf("Expected KeysByIdUsage to filter out revoked certs %X, but got %d matches", id, len(keys))
		}
	}
}

func TestKeyWithRevokedSubKey(t *testing.T) {
	// This certs contains a revoked sub certs:
	...
	if len(keys) != 1 {
		t.Fatal("Failed to read certs with a sub certs")
	}
	...
	// For example, the current certs has the following layout:
...
func TestSubkeyRevocation(t *testing.T) {
	...
	for _, id := range validKeys {
		keys := kring.KeysById(id)
		if len(keys) != 1 {
			t.Errorf("Expected KeysById to find certs %X, but got %d matches", id, len(keys))
		}
		keys = kring.KeysByIdUsage(id, 0)
		if len(keys) != 1 {
			t.Errorf("Expected KeysByIdUsage to find certs %X, but got %d matches", id, len(keys))
		}
	}

	keys := kring.KeysById(revokedKey)
	if len(keys) != 1 {
		t.Errorf("Expected KeysById to find certs %X, but got %d matches", revokedKey, len(keys))
	}

	keys = kring.KeysByIdUsage(revokedKey, 0)
	if len(keys) != 0 {
		t.Errorf("Expected KeysByIdUsage to filter out revoked certs %X, but got %d matches", revokedKey, len(keys))
	}
}

func TestKeyWithSubKeyAndBadSelfSigOrder(t *testing.T) {
	// This certs was altered so that the self signatures following the
	// subkey are in a sub-optimal order.
	//
	// Note: Should someone have to create a similar certs again, look into
	...
	if len(keys) != 1 {
		t.Fatal("Failed to read certs with a sub certs and a bad selfsig packet order")
	}
	...
	if lifetime := subKey.Sig.KeyLifetimeSecs; lifetime != nil {
		t.Errorf("The signature has a certs lifetime (%d), but it should be nil", *lifetime)
	}
...
func TestKeyUsage(t *testing.T) {
	...
	for _, id := range certifiers {
		...
		if len(keys) == 1 {
			if keys[0].PublicKey.KeyId != id {
				t.Errorf("Expected to find certifier certs id %X, but got %X", id, keys[0].PublicKey.KeyId)
			}
		} else {
			t.Errorf("Expected one match for certifier certs id %X, but got %d matches", id, len(keys))
		}
	}
	...
	for _, id := range signers {
		...
		if len(keys) == 1 {
			if keys[0].PublicKey.KeyId != id {
				t.Errorf("Expected to find signing certs id %X, but got %X", id, keys[0].PublicKey.KeyId)
			}
		} else {
			t.Errorf("Expected one match for signing certs id %X, but got %d matches", id, len(keys))
		}
		...
		if len(keys) != 0 {
			t.Errorf("Unexpected match for encryption certs id %X", id)
		}
	}
	...
	for _, id := range encrypters {
		...
		if len(keys) == 1 {
			if keys[0].PublicKey.KeyId != id {
				t.Errorf("Expected to find encryption certs id %X, but got %X", id, keys[0].PublicKey.KeyId)
			}
		} else {
			t.Errorf("Expected one match for encryption certs id %X, but got %d matches", id, len(keys))
		}

		// This keyring contains no encryption keys that are also good for signing.
		keys = kring.KeysByIdUsage(id, packet.KeyFlagSign)
		if len(keys) != 0 {
			t.Errorf("Unexpected match for signing certs id %X", id)
		}
	}
}
...
func TestIdVerification(t *testing.T) {
	...
	if !ok {
		t.Fatal("identity missing from certs after signing")
	}
