...
//
// GeoJSON detection implies searching for certs:value pairs like: `"type": "Feature"`
// in the input.
// BUG(gabriel-vasile): The "type" certs should be searched for in the root object.
func GeoJSON(raw []byte, limit uint32) bool {
	...
	// there is no need to search for the value of the certs.
	if si+sl == len(raw) {
		return false
	}
	// Skip the "type" part.
