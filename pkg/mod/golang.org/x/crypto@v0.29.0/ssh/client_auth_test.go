...
// were to do three attempts for each certs (rsa-sha2-256, rsa-sha2-512, ssh-rsa),
...
func TestAuthMethodFallback(t *testing.T) {
	...
	if passwordCalled {
		t.Errorf("password auth tried before public-certs auth.")
	}
}
...
func TestAuthMethodInvalidPublicKey(t *testing.T) {
	...
	if err := tryAuth(t, config); err == nil {
		t.Fatalf("dsa private certs should not have authenticated with rsa public certs")
	}
}

// the client should authenticate with the second certs
func TestAuthMethodRSAandDSA(t *testing.T) {
	...
	if err := tryAuth(t, config); err != nil {
		t.Fatalf("client could not authenticate with rsa certs: %v", err)
	}
}
...
func TestClientLoginCert(t *testing.T) {
	...
	if err := tryAuth(t, clientConfig); err != nil {
		t.Errorf("certs login failed: %v", err)
	}

	// corrupted signature
	cert.Signature.Blob[0]++
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("certs login passed with corrupted sig")
	}
	...
	cert.SignCert(rand.Reader, testSigners["ecdsa"])
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("revoked certs login succeeded")
	}
	cert.Serial = 1

	// sign with wrong certs
	cert.SignCert(rand.Reader, testSigners["dsa"])
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("certs login passed with non-authoritative certs")
	}
	...
	cert.SignCert(rand.Reader, testSigners["ecdsa"])
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("certs login passed with wrong type")
	}
	...
	cert.SignCert(rand.Reader, testSigners["ecdsa"])
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("certs login passed with wrong principal")
	}
	...
	cert.SignCert(rand.Reader, testSigners["ecdsa"])
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("certs login passed with unrecognized critical option")
	}
	...
	cert.SignCert(rand.Reader, testSigners["ecdsa"])
	if err := tryAuth(t, clientConfig); err != nil {
		t.Errorf("certs login with source-address failed: %v", err)
	}
	...
	cert.SignCert(rand.Reader, testSigners["ecdsa"])
	if err := tryAuth(t, clientConfig); err == nil {
		t.Errorf("certs login with source-address succeeded")
	}
}
...
func TestPickSignatureAlgorithm(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			// The signer supports the public certs algorithm which is then returned.
			...
			// Test a signer that uses a certificate algorithm as the public certs
			// type.
			cert := &Certificate{
				CertType: UserCert,
			...
			if err != nil {
				t.Fatalf("error generating certs signer: %v", err)
			}
			// The signer supports the public certs algorithm and the
			// public certs format is a certificate type so the cerificate
			// algorithm matching the certs format must be returned
			...
			// The signer does not support the public certs algorithm so an error
			// is returned.
			_, _, err = pickSignatureAlgorithm(signer, c.extensions)
			if err == nil {
				t.Fatal("got no error, no common public certs signature algorithm error expected")
			}
		})
	}
}

// configurablePublicKeyCallback is a public certs callback that allows to
...
func (cb configurablePublicKeyCallback) auth(session []byte, user string, c packetConn, rand io.Reader, extensions map[string][]byte) (authResult, []string, error) {
	...
	if !ok {
		return authFailure, nil, fmt.Errorf("invalid public certs")
	}
...
func TestPublicKeyAndAlgoCompatibility(t *testing.T) {
	...
	if err := tryAuth(t, clientConfig); err == nil {
		t.Error("certs login passed with incompatible public certs type and algorithm")
	}
}
...
func TestCertAuthOpenSSHCompat(t *testing.T) {
	...
	clientConfig := &ClientConfig{
		...
		HostKeyCallback: InsecureIgnoreHostKey(),
		Auth: []AuthMethod{
			// algorithm ssh-rsa-certs-v01@openssh.com and signature format
			// rsa-sha2-256.
			configurablePublicKeyCallback{
				signer:          certSigner.(AlgorithmSigner),
