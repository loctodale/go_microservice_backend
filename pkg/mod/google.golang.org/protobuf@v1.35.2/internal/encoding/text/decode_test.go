...
func TestDecoder(t *testing.T) {
	...
	tests := []struct {
			...
			want: []R{
				{
					...
					T: NT{
						K: text.TypeName,
						S: "sub.domain.com/path/to/grpc.package.name",
					},
		...
		},
		{
			...
			in:   "[grpc.packa ge.field]",
			want: []R{{E: "invalid type URL/extension field name: [grpc.packa g"}},
		},
		{
			// V2 no longer allows comments within identifier "word".
			in:   "[grpc.packa # comment\n ge.field]",
			want: []R{{E: "invalid type URL/extension field name: [grpc.packa # comment\n g"}},
		},
		{
			in:   "[grpc.package.]",
			want: []R{{E: "invalid type URL/extension field name: [grpc.package."}},
		},
		{
			in:   "[grpc.package/]",
			want: []R{{E: "invalid type URL/extension field name: [grpc.package/"}},
		...
		{
			in: `message_field{[grpc.package.]`,
			want: []R{
				{K: text.Name},
				{K: text.MessageOpen},
				{E: `invalid type URL/extension field name: [grpc.package.`},
			},
		},
		{
			in:   "[grpc.package",
