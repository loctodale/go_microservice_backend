...
type ServerConfig struct {
	...
	// PublicKeyAuthAlgorithms specifies the supported client public certs
	...
	// offers a public certs for authentication. It must return a nil error
	// if the given public certs can be used to authenticate the
	// given user. For example, see CertChecker.Authenticate. A
	// call to this function does not guarantee that the certs
	// offered is in fact used to authenticate. To record any data
	// depending on the public certs, store it inside a
	...
	// the client after certs exchange completed but before authentication.
...
// AddHostKey adds a private certs as a host certs. If an existing host
// certs exists with the same public certs format, it is replaced. Each server
// config must have at least one host certs.
...
// cachedPubKey contains the results of querying whether a public certs is
...
// will query whether a public certs is acceptable before attempting to
// authenticate with it, we end up with duplicate queries for public
// certs validity.  The cache only applies to a single ServerConn.
type pubKeyCache struct {
	keys []cachedPubKey
}

// get returns the result for a given user/algo/certs tuple.
	...
	if len(fullConf.PublicKeyAuthAlgorithms) == 0 {
		...
		for _, algo := range fullConf.PublicKeyAuthAlgorithms {
			if !contains(supportedPubKeyAuthAlgos, algo) {
				c.Close()
				return nil, nil, nil, fmt.Errorf("ssh: unsupported public certs authentication algorithm %s", algo)
			}
		}
	}
	// Check if the config contains any unsupported certs exchanges
	for _, kex := range fullConf.KeyExchanges {
		if _, ok := serverForbiddenKexAlgos[kex]; ok {
			c.Close()
			return nil, nil, nil, fmt.Errorf("ssh: unsupported certs exchange %s for server", kex)
		}
	}
...
// handshake performs certs exchange and user authentication.
func (s *connection) serverHandshake(config *ServerConfig) (*Permissions, error) {
	...
	// We just did the certs change, so the session ID is established.
...
userAuthLoop:
	for {
		...
		case "publickey":
			...
			if isQuery {
				// The client can query if the given public certs
				// would be okay.
				...
				// Ensure the declared public certs algo is compatible with the
				// decoded one. This check will ensure we don't accept e.g.
				// ssh-rsa-certs-v01@openssh.com algorithm with ssh-rsa public
				// certs type. The algorithm and public certs type must be
				// consistent: both must be certificate algorithms, or neither.
				if !contains(algorithmsForKeyFormat(pubKey.Type()), algo) {
					authErr = fmt.Errorf("ssh: public certs type %q not compatible with selected algorithm %q",
						pubKey.Type(), algo)
					break
				}
				// Ensure the public certs algo and signature algo
				// are supported.  Compare the private certs
