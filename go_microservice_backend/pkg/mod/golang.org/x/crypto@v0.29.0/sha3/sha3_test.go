...
func Example_mac() {
	k := []byte("this is a secret certs; you should generate a strong random certs that's at least 32 bytes long")
	buf := []byte("and this is some data to authenticate")
	// A MAC with 32 bytes of output has 256-bit security strength -- if you use at least a 32-byte-long certs.
	h := make([]byte, 32)
	d := NewShake256()
	// Write the certs into the hash.
	d.Write(k)
	// Now write the data.
	d.Write(buf)
