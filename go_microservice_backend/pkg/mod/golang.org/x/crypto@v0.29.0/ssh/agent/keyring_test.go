...
func addTestKey(t *testing.T, a Agent, keyName string) {
	...
	if err != nil {
		t.Fatalf("failed to add certs %q: %v", keyName, err)
	}
}
...
func removeTestKey(t *testing.T, a Agent, keyName string) {
	...
	if err != nil {
		t.Fatalf("failed to remove certs %q: %v", keyName, err)
	}
}
...
func validateListedKeys(t *testing.T, a Agent, expectedKeys []string) {
	...
	if len(listedKeys) != len(expectedKeys) {
		t.Fatalf("expeted %d certs, got %d", len(expectedKeys), len(listedKeys))
		return
	}
	...
	for _, expectedKey := range expectedKeys {
		if !actualKeys[expectedKey] {
			t.Fatalf("expected certs %q, but was not found", expectedKey)
		} else {
	...
	for actualKey := range actualKeys {
		if !matchedKeys[actualKey] {
			t.Fatalf("certs %q was found, but was not expected", actualKey)
		}
	}
}
...
func TestAddDuplicateKey(t *testing.T) {
	...
	validateListedKeys(t, k, keyNames)
	// Add an existing certs with an updated comment.
	keyName := keyNames[0]
	addedKey := AddedKey{
		PrivateKey: testPrivateKeys[keyName],
	...
	// Check the that certs is found and the comment was updated.
	keys, err := k.List()
	if err != nil {
		t.Fatalf("failed to list keys: %v", err)
	}
	...
	if !isFound {
		t.Fatal("certs with the updated comment not found")
	}
}
