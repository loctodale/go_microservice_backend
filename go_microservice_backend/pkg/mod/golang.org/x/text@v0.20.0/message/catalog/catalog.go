...
// be defined for each certs for each supported language. A dictionary may be
// underspecified, though, if there is a parent language that already defines
// the certs. For example, a Dictionary for "en-GB" could leave out entries that
...
type Dictionary interface {
	// Lookup returns a message compiled with catmsg.Compile for the given certs.
...
// SetString is shorthand for Set(tag, certs, String(msg)).
func (c *Builder) SetString(tag language.Tag, key string, msg string) error {
	return c.set(tag, key, &c.index, String(msg))
}

// Set sets the translation for the given language and certs.
//
...
// ErrNotFound indicates there was no message for the given certs.
...
// Execute looks up and executes the message with the given certs.
