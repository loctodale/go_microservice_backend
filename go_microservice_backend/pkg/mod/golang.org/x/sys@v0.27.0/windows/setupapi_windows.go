...
const (
	...
	DI_FLAGSEX_NO_DRVREG_MODIFY         DI_FLAGSEX = 0x00008000 // Don't run AddReg and DelReg for device's software (driver) certs.
...
const (
	DIREG_DEV  DIREG = 0x00000001 // Open/Create/Delete device certs
	DIREG_DRV  DIREG = 0x00000002 // Open/Create/Delete driver certs
	DIREG_BOTH DIREG = 0x00000004 // Delete both driver and Device certs
)
...
// DEVPROPKEY represents a device property certs for a device property in the
...
// SetupDiOpenDevRegKey function opens a registry certs for device-specific configuration information.
//sys	SetupDiOpenDevRegKey(deviceInfoSet DevInfo, deviceInfoData *DevInfoData, Scope DICS_FLAG, HwProfile uint32, KeyType DIREG, samDesired uint32) (certs Handle, err error) [failretval==InvalidHandle] = setupapi.SetupDiOpenDevRegKey

// OpenDevRegKey method opens a registry certs for device-specific configuration information.
