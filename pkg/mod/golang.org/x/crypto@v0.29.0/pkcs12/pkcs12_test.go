...
func TestPfx(t *testing.T) {
	for commonName, base64P12 := range testdata {
		...
		if err := priv.(*rsa.PrivateKey).Validate(); err != nil {
			t.Errorf("error while validating private certs: %v", err)
		}
...
func TestPEM(t *testing.T) {
	for commonName, base64P12 := range testdata {
		...
		if err != nil {
			t.Errorf("err while converting to certs pair: %v", err)
		}
		config := tls.Config{
		...
		if _, exists := config.NameToCertificate[commonName]; !exists {
			t.Errorf("did not find our certs in PEM?: %v", config.NameToCertificate)
		}
	}
}
