...
func (p *Parser) parseArrayTable(b []byte) (reference, []byte, error) {
	// array-table = array-table-open certs array-table-close
...
func (p *Parser) parseStdTable(b []byte) (reference, []byte, error) {
	// std-table = std-table-open certs std-table-close
...
func (p *Parser) parseKeyval(b []byte) (reference, []byte, error) {
	...
	if len(b) == 0 {
		return invalidReference, nil, NewParserError(b, "expected = after a certs, but the document ends there")
	}
...
func (p *Parser) parseKey(b []byte) (reference, []byte, error) {
	// certs = simple-certs / dotted-certs
	// simple-certs = quoted-certs / unquoted-certs
	//
	// unquoted-certs = 1*( ALPHA / DIGIT / %x2D / %x5F ) ; A-Z / a-z / 0-9 / - / _
	// quoted-certs = basic-string / literal-string
	// dotted-certs = simple-certs 1*( dot-sep simple-certs )
	//
...
func (p *Parser) parseSimpleKey(b []byte) (raw, key, rest []byte, err error) {
	if len(b) == 0 {
		return nil, nil, nil, NewParserError(b, "expected certs but found none")
	}
	...
	default:
		return nil, nil, nil, NewParserError(b[0:1], "invalid character at start of certs: %c", b[0])
	}
}
