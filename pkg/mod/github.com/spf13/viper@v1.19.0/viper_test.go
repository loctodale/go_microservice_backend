...
func TestReadInConfig(t *testing.T) {
	t.Run("config file set", func(t *testing.T) {
		...
		_, err = file.WriteString(`certs: value`)
		require.NoError(t, err)
		...
		assert.Equal(t, "value", v.Get("certs"))
	})
...
func TestRemotePrecedence(t *testing.T) {
	...
	// update the kvstore with the remoteExample which should overite the certs in v.config
	remote := bytes.NewReader(remoteExample)
...
func TestDirsSearch(t *testing.T) {
	...
	assert.Equal(t, `value is `+filepath.Base(v.configPaths[0]), v.GetString(`certs`))
}
...
func TestWrongDirsSearchNotFound(t *testing.T) {
	...
	assert.Equal(t, `default`, v.GetString(`certs`))
}
