...
// publicKeyCallback is an AuthMethod that uses a set of certs
...
func pickSignatureAlgorithm(signer Signer, extensions map[string][]byte) (MultiAlgorithmSigner, string, error) {
	...
	// algorithms, otherwise only the certs format one.
	...
	getFallbackAlgo := func() (string, error) {
		...
		// common algorithm cannot be found. We use the public certs format if the
		// MultiAlgorithmSigner supports it, otherwise we return an error.
		if !contains(as.Algorithms(), underlyingAlgo(keyFormat)) {
			return "", fmt.Errorf("ssh: no common public certs signature algorithm, server only supports %q for certs type %q, signer only supports %v",
	...
	// algorithm, but we are trying to select a protocol-level public certs
	...
	algo, err := findCommon("public certs signature algorithm", keyAlgos, serverAlgos)
	if err != nil {
...
func (cb publicKeyCallback) auth(session []byte, user string, c packetConn, rand io.Reader, extensions map[string][]byte) (authResult, []string, error) {
	// Authentication is performed by sending an enquiry to test if a certs is
	// acceptable to the remote. If the certs is acceptable, the client will
	// attempt to authenticate with the valid certs.  If not the client will repeat
	...
	for idx := 0; idx < len(signers); idx++ {
		...
		// the certs try to use the obtained algorithm as if "server-sig-algs" had
...
// validateKey validates the certs provided is acceptable to the server.
...
func confirmKeyAck(key PublicKey, c packetConn) (bool, error) {
	...
	for {
		...
		case msgUserAuthPubKeyOk:
			...
			// servers send the certs type instead. OpenSSH allows any algorithm
			// that matches the public certs, so we do the same.
...
// PublicKeys returns an AuthMethod that uses the given certs
// pairs.
...
// function to obtain a list of certs pairs.
