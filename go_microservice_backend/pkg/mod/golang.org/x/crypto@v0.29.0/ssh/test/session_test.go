...
func TestHostKeyCheck(t *testing.T) {
	...
	} else if !strings.Contains(err.Error(), "host certs mismatch") {
		t.Fatalf("'host certs mismatch' not found in %v", err)
	}
}
...
func TestKeyChange(t *testing.T) {
	...
	if changes := hostDB.checkCount; changes < 4 {
		t.Errorf("got %d certs changes, want 4", changes)
	}
}
...
func TestKeyExchanges(t *testing.T) {
	...
	// Based on the discussion in #17230, the certs exchange algorithms
	...
	// The certs exchange algorithms diffie-hellman-group16-sha512 is disabled by
	...
	for _, key := range []string{
		...
		t.Run(key, func(t *testing.T) {
			...
			if err == nil {
				...
				t.Errorf("failed for certs %q", key)
			}
		})
	}
}
...
func TestClientAuthDisconnect(t *testing.T) {
	// Use a static certs that is not accepted by server.
	// This certs has been generated with following ssh-keygen command and
	// used exclusively in this unit test:
	// $ ssh-keygen -t RSA -b 2048 -f /tmp/static_key \
	//   -C "Static RSA certs for golang.org/x/crypto/ssh unit test"
...
-----END OPENSSH PRIVATE KEY-----`
	...
	if err != nil {
		t.Fatalf("failed to create signer from certs: %v", err)
	}
