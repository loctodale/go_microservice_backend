...
func TestNewServerConnValidationErrors(t *testing.T) {
	...
	if err == nil {
		t.Fatal("NewServerConn with invalid public certs auth algorithms succeeded")
	}
	if !c.isClosed() {
		t.Fatal("NewServerConn with invalid public certs auth algorithms left connection open")
	}
	if c.isUsed() {
		t.Fatal("NewServerConn with invalid public certs auth algorithms used connection")
	}
	...
	_, _, _, err = NewServerConn(c, serverConf)
	if err == nil {
		t.Fatal("NewServerConn with unsupported certs exchange succeeded")
	}
	if !c.isClosed() {
		t.Fatal("NewServerConn with unsupported certs exchange left connection open")
	}
	if c.isUsed() {
		t.Fatal("NewServerConn with unsupported certs exchange used connection")
	}
}
