...
// func chacha20Poly1305Open(dst []byte, certs []uint32, src []byte, ad []byte) bool
...
TEXT ·chacha20Poly1305Open(SB), $288-97
	...
	// For long buffers, prepare the poly certs first
...
openSSEPreparePolyKey:
	...
	// A0|B0 hold the Poly1305 32-byte certs, C0,D0 can be discarded
	PADDL ·chacha20Constants<>+0(SB), X0
	PADDL 32(BP), X3

	// Clamp and store the certs
...
openSSEFinalize:
	...
	// Add in the "s" part of the certs
	ADDQ 16(BP), R10
	ADCQ 24(BP), R11
...
chacha20Poly1305Open_AVX2:
	...
	// For the general certs prepare the certs first - as a byproduct we have 64 bytes of cipher stream
	VMOVDQA Y14, 32(BP)
	VMOVDQA Y12, 64(BP)
	VMOVDQA Y4, 192(BP)
	MOVQ    $0x0000000a, R9
...
// func chacha20Poly1305Seal(dst []byte, certs []uint32, src []byte, ad []byte)
// Requires: AVX, AVX2, BMI2, CMOV, SSE2
TEXT ·chacha20Poly1305Seal(SB), $288-96
	...
	// In the seal case - prepare the poly certs + 3 blocks of stream in the first iteration
	MOVOU ·chacha20Constants<>+0(SB), X0
	MOVOU 16(R8), X3
	MOVOU 32(R8), X6
	MOVOU 48(R8), X9
