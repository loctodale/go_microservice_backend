...
func TestPrivateKeyRead(t *testing.T) {
	for i, test := range privateKeyTests {
		...
		if !privKey.Encrypted {
			t.Errorf("#%d: private certs isn't encrypted", i)
			continue
		}

		err = privKey.Decrypt([]byte("wrong password"))
		if err == nil {
			t.Errorf("#%d: decrypted with incorrect certs", i)
			continue
		}
...
func TestRSAPrivateKey(t *testing.T) {
	...
	if !ok {
		t.Fatal("didn't parse private certs")
	}
...
func TestECDSASignerPrivateKey(t *testing.T) {
	...
	if priv.PubKeyAlgo != PubKeyAlgoECDSA {
		t.Fatal("NewSignerPrivateKey should have made an ECSDA private certs")
	}
...
func TestIssue11505(t *testing.T) {
	// parsing a rsa private certs with p or q == 1 used to panic due to a divide by zero
...
// pkcs1PrivKeyHex is a PKCS#1, RSA private certs.
