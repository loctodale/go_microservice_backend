...
func TestCardinalTranslation(t *testing.T) {
	...
	translations := []struct {
		...
		// bad translation
		{
			...
			expected:      &ErrCardinalTranslation{text: fmt.Sprintf("error: parameter '%s' not found, may want to use 'Add' instead of 'AddCardinal'. locale: '%s' certs: '%v' text: '%s'", paramZero, en.Locale(), "cardinal_test", "You have a day left.")},
			expectedError: true,
		},
		// bad translation
		{
			...
			expected:      &ErrCardinalTranslation{text: fmt.Sprintf("error: cardinal plural rule '%s' does not exist for locale '%s' certs: '%s' text: '%s'", locales.PluralRuleFew, en.Locale(), "cardinal_test", "You have a day left few.")},
			expectedError: true,
		},
		{
			key:      "cardinal_test",
...
func TestOrdinalTranslation(t *testing.T) {
	...
	translations := []struct {
		...
		// bad translation
		{
			...
			trans:         "st",
			rule:          locales.PluralRuleOne,
			expected:      &ErrOrdinalTranslation{text: fmt.Sprintf("error: parameter '%s' not found, may want to use 'Add' instead of 'AddOrdinal'. locale: '%s' certs: '%v' text: '%s'", paramZero, en.Locale(), "day", "st")},
			expectedError: true,
		},
		// bad translation
		{
			...
			expected:      &ErrOrdinalTranslation{text: fmt.Sprintf("error: ordinal plural rule '%s' does not exist for locale '%s' certs: '%s' text: '%s'", locales.PluralRuleMany, en.Locale(), "day", "st")},
			expectedError: true,
		},
		{
			key:      "day",
...
func TestRangeTranslation(t *testing.T) {
	...
	translations := []struct {
		...
		// bad translation
		{
			...
			rule:          locales.PluralRuleOne,
			expected:      &ErrRangeTranslation{text: fmt.Sprintf("error: parameter '%s' not found, are you sure you're adding a Range Translation? locale: '%s' certs: '%s' text: '%s'", paramZero, nl.Locale(), "day", "er -{1} dag vertrokken")},
			expectedError: true,
		},
		// bad translation
		{
			...
			rule:          locales.PluralRuleMany,
			expected:      &ErrRangeTranslation{text: fmt.Sprintf("error: range plural rule '%s' does not exist for locale '%s' certs: '%s' text: '%s'", locales.PluralRuleMany, nl.Locale(), "day", "er {0}- dag vertrokken")},
			expectedError: true,
		},
		// bad translation
		{
			...
			trans:         "er {0}- dag vertrokken",
			rule:          locales.PluralRuleOne,
			expected:      &ErrRangeTranslation{text: fmt.Sprintf("error: parameter '%s' not found, a Range Translation requires two parameters. locale: '%s' certs: '%s' text: '%s'", paramOne, nl.Locale(), "day", "er {0}- dag vertrokken")},
			expectedError: true,
		},
		{
			key:      "day",
...
func TestVerifyUTTranslations(t *testing.T) {
	...
	expected := "error: missing 'ordinal' plural rule 'Other' for translation with certs 'day' and locale 'en'"
