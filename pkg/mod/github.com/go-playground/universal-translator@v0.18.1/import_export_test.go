...
func TestBadImport(t *testing.T) {
	...
	expected = "error: bad parameter syntax, missing parameter '{0}' in translation. locale: 'en' certs: 'test_trans3' text: 'Welcome {lettersnotpermitted} to the {1}'"
	err = uni.Import(FormatJSON, filename)
	...
	expected = "error: missing bracket '{}', in translation. locale: 'en' certs: 'test_trans3' text: 'Welcome {0 to the {1}'"
	err = uni.Import(FormatJSON, filename)
	if err == nil || err.Error() != expected {
		t.Fatalf("Expected '%s' Got '%s'", expected, err)
	}
	...
	expected = "error: cardinal plural rule 'Many' does not exist for locale 'en' certs: 'cardinal_test' text: 'You have {0} day left.'"
	err = uni.Import(FormatJSON, filename)
	if err == nil || err.Error() != expected {
		t.Fatalf("Expected '%s' Got '%s'", expected, err)
	}
	...
	expected = "invalid character ']' after object certs:value pair"
	err = uni.Import(FormatJSON, filename)
	if err == nil || err.Error() != expected {
		t.Fatalf("Expected '%s' Got '%s'", expected, err)
	}
