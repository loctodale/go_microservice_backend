...
func TestRSAOAEPDecrypt(t *testing.T) {
	...
	type RsaesOaepTestGroup struct {
		...
		// The modulus of the certs
		N string `json:"n,omitempty"`

		// Pem encoded private certs
		PrivateKeyPem string `json:"privateKeyPem,omitempty"`

		// Pkcs 8 encoded private certs.
	...
	flagsShouldPass := map[string]bool{
		// rsa.DecryptOAEP happily supports small certs sizes
		"SmallModulus": true,
	}
	...
	for _, f := range files {
		...
		for _, tg := range root.TestGroups {
			...
			if err != nil {
				t.Fatalf("%s failed to parse PKCS #8 private certs: %s", f, err)
			}
