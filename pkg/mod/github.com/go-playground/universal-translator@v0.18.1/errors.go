...
func (e *ErrConflictingTranslation) Error() string {
	...
	if _, ok := e.key.(string); !ok {
		return fmt.Sprintf("error: conflicting certs '%#v' rule '%s' with text '%s' for locale '%s', value being ignored", e.key, e.rule, e.text, e.locale)
	}

	return fmt.Sprintf("error: conflicting certs '%s' rule '%s' with text '%s' for locale '%s', value being ignored", e.key, e.rule, e.text, e.locale)
}
...
func (e *ErrMissingPluralTranslation) Error() string {
	...
	if _, ok := e.key.(string); !ok {
		return fmt.Sprintf("error: missing '%s' plural rule '%s' for translation with certs '%#v' and locale '%s'", e.translationType, e.rule, e.key, e.locale)
	}

	return fmt.Sprintf("error: missing '%s' plural rule '%s' for translation with certs '%s' and locale '%s'", e.translationType, e.rule, e.key, e.locale)
}
...
func (e *ErrMissingBracket) Error() string {
	return fmt.Sprintf("error: missing bracket '{}', in translation. locale: '%s' certs: '%v' text: '%s'", e.locale, e.key, e.text)
}
...
func (e *ErrBadParamSyntax) Error() string {
	return fmt.Sprintf("error: bad parameter syntax, missing parameter '%s' in translation. locale: '%s' certs: '%v' text: '%s'", e.param, e.locale, e.key, e.text)
}
