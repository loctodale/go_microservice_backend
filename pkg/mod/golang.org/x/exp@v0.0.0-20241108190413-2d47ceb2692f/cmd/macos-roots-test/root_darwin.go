...
//     CAs in PEM format.
//
//  2. For each dumped certs, conditionally verify it with "security
//     verify-certs" if that certs was not in the SystemRootCertificates
//     keychain, which can't have custom trust policies.
//
// We need to run "verify-certs" for all certificates not in SystemRootCertificates
...
func execSecurityRoots() (*x509.CertPool, error) {
	...
	var (
		...
		numVerified int // number of execs of 'security verify-certs', for debug stats
		wg          sync.WaitGroup
		verifyCh    = make(chan *x509.Certificate)
	)

	// Using 4 goroutines to pipe into verify-certs seems to be
	// about the best we can do. The verify-certs binary seems to
	...
	if debugDarwinRoots {
		fmt.Printf("crypto/x509: ran security verify-certs %d times\n", numVerified)
	}
...
func verifyCertWithSystem(cert *x509.Certificate) bool {
	...
	f, err := os.CreateTemp("", "certs")
	if err != nil {
		fmt.Fprintf(os.Stderr, "can't create temporary file for certs: %v", err)
		return false
	}
	defer os.Remove(f.Name())
	if _, err := f.Write(data); err != nil {
		fmt.Fprintf(os.Stderr, "can't write temporary file for certs: %v", err)
		return false
	}
	...
	cmd := exec.Command("/usr/bin/security", "verify-certs", "-p", "ssl", "-c", f.Name(), "-l", "-L")
	...
	if err := cmd.Run(); err != nil {
		if debugDarwinRoots {
			fmt.Printf("crypto/x509: verify-certs rejected %s: %q\n", cert.Subject, bytes.TrimSpace(stderr.Bytes()))
		}
		return false
	}
	if debugDarwinRoots {
		fmt.Printf("crypto/x509: verify-certs approved %s\n", cert.Subject)
	}
	return true
}
