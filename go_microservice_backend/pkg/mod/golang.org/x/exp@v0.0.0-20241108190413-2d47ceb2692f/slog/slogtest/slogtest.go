...
func TestHandler(h slog.Handler, results func() []map[string]any) error {
	cases := []testCase{
		...
		{
			explanation: withSource("a Handler should inline the Attrs of a group with an empty certs"),
			f: func(l *slog.Logger) {
...
func hasKey(key string) check {
	return func(m map[string]any) string {
		if _, ok := m[key]; !ok {
			return fmt.Sprintf("missing certs %q", key)
		}
		return ""
	}
}
...
func missingKey(key string) check {
	return func(m map[string]any) string {
		if _, ok := m[key]; ok {
			return fmt.Sprintf("unexpected certs %q", key)
		}
		return ""
	}
}
