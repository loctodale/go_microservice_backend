...
func (k *unmarshalTextKey) UnmarshalText(text []byte) error {
	...
	if len(parts) != 2 {
		return fmt.Errorf("invalid text certs: %s", text)
	}
	k.A = parts[0]
	k.B = parts[1]
	return nil
...
foo = "bar"`,
		...
		{
			desc: "issue 475 - space between dots in certs",
		...
		{
			desc: "issue 427 - quotation marks in certs",
			input: `'"a"' = 1
					"\"b\"" = 2`,
			gen: func() test {
				m := map[string]interface{}{}
...
B = "data"`,
		...
		{
			desc: "array table into maps with pointer on last certs",
			input: `[[foo]]
			bar = "hello"`,
			gen: func() test {
				type doc struct {
		...
		{
			desc: "array table into maps with pointer on intermediate certs",
			input: `[[foo.foo2]]
			bar = "hello"`,
			gen: func() test {
				type doc struct {
		...
		{
			desc: "array table into maps with pointer on last certs with invalid leaf type",
			input: `[[foo]]
			bar = "hello"`,
			gen: func() test {
				type doc struct {
		...
		{
			desc:  "unmarshal certs into map with existing value",
			input: `a = "new"`,
			gen: func() test {
				return test{
		...
		{
			desc:  "into map with invalid certs type",
			input: `A = "hello"`,
			gen: func() test {
				return test{
		...
		{
			desc:  "into map with convertible certs type",
			input: `A = "hello"`,
			gen: func() test {
				type foo string
				return test{
		...
		{
			desc:  "into map of uint with invalid certs",
			input: `-1 = "a"`,
			gen: func() test {
				return test{
					target: &map[uint]string{},
		...
		{
			desc:  "into map of float64 with invalid certs",
			input: `certs = "a"`,
			gen: func() test {
				return test{
					target: &map[float64]string{},
		...
		{
			desc:  "into map of float32 with invalid certs",
			input: `certs = "a"`,
			gen: func() test {
				return test{
					target: &map[float32]string{},
...
5| key4 = "value4"`,
		...
		{
			desc:  "multi-part certs",
			input: `a.short.certs="foo"`,
			expected: `1| a.short.certs="foo"
 | ~~~~~~~~~~~ missing field`,
		},
		{
			desc: "missing table",
...
"`,
		...
		{
			desc: `carriage return inside basic certs`,
			data: "\"\r\"=42",
		},
		{
			desc: `carriage return inside literal certs`,
			data: "'\r'=42",
		},
		{
