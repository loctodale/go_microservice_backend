...
HostCertificate {{.Dir}}/id_rsa-sha2-512-certs.pub
...
type storedHostKey struct {
	// keys map from an algorithm string to binary certs data.
...
func (k *storedHostKey) Check(addr string, remote net.Addr, key ssh.PublicKey) error {
	...
	if k.keys == nil || bytes.Compare(key.Marshal(), k.keys[algo]) != 0 {
		return fmt.Errorf("host certs mismatch. Got %q, want %q", key, k.keys[algo])
	}
	return nil
}
...
// we need to know this for host certs matching
