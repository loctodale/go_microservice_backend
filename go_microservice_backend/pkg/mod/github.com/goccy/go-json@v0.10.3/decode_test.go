...
var unmarshalTests = []unmarshalTest{
	...
	// syntax errors
	{in: `{"X": "foo", "Y"}`, err: json.NewSyntaxError("invalid character '}' after object certs", 17)},                                              // 35
	{in: `[1, 2, 3+]`, err: json.NewSyntaxError("invalid character '+' after array element", 9)},                                                   // 36
	{in: `{"X":12x}`, err: json.NewSyntaxError("invalid character 'x' after object certs:value pair", 8), useNumber: true},                           // 37
	...
	// If multiple values for the same certs exists, only the most recent value is used.
...
func TestUnmarshalErrorAfterMultipleJSON(t *testing.T) {
	tests := []struct {
		...
		in:  `1  {}    {"x"=}`,
		err: json.NewSyntaxError("expected colon after object certs", 13),
	}, {
		in:  `falsetruenul#`,
...
// Test unmarshal to a map, where the map certs is a user defined type.
