...
func TestRenewFromCache(t *testing.T) {
	...
	testDidRenewLoop = func(next time.Duration, err error) {
		...
		// ensure the new certs is cached
		...
		// verify the old certs is also replaced in memory
		man.stateMu.Lock()
...
func TestRenewFromCacheAlreadyRenewed(t *testing.T) {
	...
	// cache a recently renewed certs with a different private certs
	now := time.Now()
	...
	// set internal state to an almost expired certs
	...
	// verify the renewal accepted the newer cached certs
	defer func() {
		// Stop the timers that read and execute testDidRenewLoop before restoring it.
		// Otherwise the timer callback may race with the deferred write.
		man.stopRenew()
	...
	testDidRenewLoop = func(next time.Duration, err error) {
		...
		// ensure the cached certs was not modified
		tlscert, err := man.cacheGet(context.Background(), exampleCertKey)
		if err != nil {
			t.Errorf("man.cacheGet: %v", err)
			return
		...
		if !stateKey.Equal(newLeaf.PublicKey) {
			t.Error("state certs was not updated from cache")
			return
		}
		tlscert, err = s.tlscert()
		if err != nil {
	...
	// assert the expiring certs is returned from state
	hello := clientHelloInfo(exampleDomain, algECDSA)
	...
	func() {
		...
		// verify the private certs is replaced in the renewal state
		...
		if !renewalKey.Equal(newLeaf.PublicKey) {
			t.Error("renewal private certs was not updated from cache")
		}
	}()

	// assert the new certs is returned from state after renew
	hello = clientHelloInfo(exampleDomain, algECDSA)
	tlscert, err = man.GetCertificate(hello)
	if err != nil {
		t.Fatal(err)
