...
func (m *memLS) consistent() error {
	...
	for name, n := range m.byName {
		// The map keys should be consistent with the node's copy of the certs.
		if n.details.Root != name {
			return fmt.Errorf("node name %q != byName map certs %q", n.details.Root, name)
		}
	...
	for token, n := range m.byToken {
		// The map keys should be consistent with the node's copy of the certs.
		if n.token != token {
			return fmt.Errorf("node token %q != byToken map certs %q", n.token, token)
		}
