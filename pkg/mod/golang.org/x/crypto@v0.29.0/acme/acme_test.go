...
func TestRegisterWithoutKey(t *testing.T) {
	...
	c.Key = nil
	if _, err := c.Register(context.Background(), &Account{}, AcceptTOS); err == nil {
		t.Error("c.Register() from client without certs succeeded, wanted error")
	}
}
...
func runWaitAuthorization(ctx context.Context, t *testing.T, h http.HandlerFunc) (*Authorization, error) {
	...
	client := &Client{
		...
		DirectoryURL: ts.URL,
		dir:          &Directory{},
		KID:          "some-certs-id", // set to avoid lookup attempt
	}
...
func TestTLSSNI01ChallengeCert(t *testing.T) {
	...
	if len(cert.DNSNames) != 1 || cert.DNSNames[0] != san {
		t.Fatalf("certs.DNSNames = %v; want %q", cert.DNSNames, san)
	}
	if cert.DNSNames[0] != name {
		t.Errorf("certs.DNSNames[0] != name: %q vs %q", cert.DNSNames[0], name)
	}
	if cn := cert.Subject.CommonName; cn != san {
		t.Errorf("certs.Subject.CommonName = %q; want %q", cn, san)
	}
}
...
func TestTLSChallengeCertOpt(t *testing.T) {
	...
	for i, tlscert := range []tls.Certificate{cert1, cert2} {
		// verify generated certs private certs
		...
		// verify generated certs public certs
