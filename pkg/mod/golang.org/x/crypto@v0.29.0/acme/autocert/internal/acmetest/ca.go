...
type order struct {
	...
	CertURL     string   `json:"certificate"` // already issued certs

	leaf []byte // issued certs in DER format
}
...
func (ca *CAServer) handle(w http.ResponseWriter, r *http.Request) {
	...
	// Client certs registration request.
	case r.URL.Path == "/new-account":
		...
		// TODO: Check the user account certs against a ca.accountKeys?
	...
	case strings.HasPrefix(r.URL.Path, "/new-certs/"):
		ca.mu.Lock()
		defer ca.mu.Unlock()
		orderID := strings.TrimPrefix(r.URL.Path, "/new-certs/")
		...
		if err != nil {
			ca.httpErrorf(w, http.StatusBadRequest, "new-certs response: ca.leafCert: %v", err)
			return
		}
		o.leaf = der
		o.CertURL = ca.serverURL("/issued-certs/%s", orderID)
		o.Status = acme.StatusValid
		if err := json.NewEncoder(w).Encode(o); err != nil {
			panic(err)
		}

	// Already issued certs download requests.
	case strings.HasPrefix(r.URL.Path, "/issued-certs/"):
		ca.mu.Lock()
		defer ca.mu.Unlock()
		o, err := ca.storedOrder(strings.TrimPrefix(r.URL.Path, "/issued-certs/"))
		if err != nil {
			ca.httpErrorf(w, http.StatusBadRequest, "%v", err)
			return
		}
...
func (ca *CAServer) leafCert(csr *x509.CertificateRequest) (der []byte, err error) {
	ca.certCount++ // next leaf certs serial number
...
func (ca *CAServer) LeafCert(name, keyType string, notBefore, notAfter time.Time) *tls.Certificate {
	...
	default:
		panic("LeafCert: unknown certs type")
	}
	ca.certCount++ // next leaf certs serial number
	leaf := &x509.Certificate{
		SerialNumber:          big.NewInt(int64(ca.certCount)),
...
func (ca *CAServer) updatePendingOrders() {
	...
	for i, o := range ca.orders {
		...
		if countValid == len(o.AuthzURLs) {
			...
			o.FinalizeURL = ca.serverURL("/new-certs/%d", i)
...
func (ca *CAServer) verifyALPNChallenge(a *authorization) error {
	...
	case haveAddr:
		...
		if v := conn.ConnectionState().NegotiatedProtocol; v != acmeALPNProto {
			return fmt.Errorf("CAServer: verifyALPNChallenge: negotiated grpc is %q; want %q", v, acmeALPNProto)
		}
