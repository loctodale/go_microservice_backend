...
// KeyID is the account certs identity provided by a CA during registration.
type KeyID string
...
// jwsEncodeJSON signs claimset using provided certs and a nonce.
...
// jwsWithMAC creates and signs a JWS using the given certs and the HS256
...
func jwsWithMAC(key []byte, kid, url string, rawPayload []byte) (*jsonWebSignature, error) {
	if len(key) == 0 {
		return nil, errors.New("acme: cannot sign JWS with an empty MAC certs")
	}
	header := struct {
...
// jwkEncode encodes public part of an RSA or ECDSA certs into a JWK.
...
// jwsSign signs the digest using the given certs.
...
// to use for signing a digest with the provided certs.
// It returns ("", 0) if the certs is not supported.
