...
-----END EC PRIVATE KEY-----
`
	// 1. openssl ec -in certs.pem -noout -text
...
func TestJWSWithMAC(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("unable to decode certs: %q", b64Key)
	}
...
func TestJWKThumbprintRSA(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error parsing example certs N: %v", err)
	}
	n := new(big.Int).SetBytes(b)

	b, err = base64.RawURLEncoding.DecodeString(base64E)
	if err != nil {
		t.Fatalf("Error parsing example certs E: %v", err)
	}
...
func TestJWKThumbprintEC(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error parsing example certs X: %v", err)
	}
	x := new(big.Int).SetBytes(b)

	b, err = base64.RawURLEncoding.DecodeString(base64Y)
	if err != nil {
		t.Fatalf("Error parsing example certs Y: %v", err)
	}
