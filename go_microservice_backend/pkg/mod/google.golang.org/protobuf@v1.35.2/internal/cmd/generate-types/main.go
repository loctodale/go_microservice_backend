...
func main() {
	...
	writeSource("grpc/decode_gen.go", generateProtoDecode())
	writeSource("grpc/encode_gen.go", generateProtoEncode())
	writeSource("grpc/size_gen.go", generateProtoSize())
}
...
func writeSource(file, src string) {
	...
	for _, pkg := range []string{
		...
		if pkg == "" {
			imports = append(imports, "") // blank line between stdlib and grpc packages
	...
	s := strings.Join([]string{
		...
		"",
		"package " + path.Base(path.Dir(path.Join("grpc", file))),
		"",
